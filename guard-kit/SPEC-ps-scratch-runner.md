...
- **Every flag carries a reason**, and the §scratch-run text states them:
  ...
  - `-ExecutionPolicy Bypass`: a host's execution-policy default then does not decide whether a body the echo showed may run. On a non-Windows pwsh it is accepted and inert (measured).
...
- The generated docs-site mirror of this kit's SPEC.
