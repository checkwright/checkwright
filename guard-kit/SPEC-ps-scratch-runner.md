...
No delta rests on Windows PowerShell 5.1's documented behaviour. Delta 2 keeps the `.ps1` extension and passes `-ExecutionPolicy Bypass` whatever 5.1 does without them, and delta 7's leg is 5.1's first measurement. One premise is carried unrun:
...
- The extension decides because it is how PowerShell and Windows name a script. It is read off the file the runner already holds, so no roster is involved, and `GUARD_KIT_SCRIPT_INTERPRETERS` stays unread and undeclared by the arm.
...
- **Every flag carries a reason**, and the §scratch-run text states them:
  ...
  - `-ExecutionPolicy Bypass`: it sets the policy of the host's own process, so a locally configured policy that refuses script files does not refuse the snapshot. On a non-Windows pwsh it is accepted and inert (measured). **Honest limit, carried into §scratch-run's text:** the flag sets the process scope only. Microsoft documents a Group Policy scope, machine or user, as outranking it, so on a host whose execution policy is set by Group Policy a PowerShell body can still be refused, and the runner then passes through whatever exit status the host returns. The precedence is documented, not measured: no Windows host exists here, and delta 7's leg is not built to measure it, since it sets no Group Policy.
