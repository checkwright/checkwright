...
fn rule(args: &[String]) -> Result<i32, String> {
    ...
    for f in &files {
        ...
        for (i, (line, v)) in text.lines().zip(scan(&text)).enumerate() {
            if v.soft {
...
mod tests {
    ...
    fn soft(text: &str) -> Vec<usize> {
        scan(text).iter().enumerate().filter(|(_, v)| v.soft).map(|(i, _)| i + 1).collect()
    }
    ...
    fn the_join_holds_its_postcondition_over_both_fixture_trees() {
        ...
        for case in ["good", "bad"] {
            ...
            assert!(soft(&unwrap(&text)).is_empty(), "{}", p.display());
        }
    }
}
