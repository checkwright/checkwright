...
// spec: gate-sdk/SPEC.md §check-action-run-path — a single-line value's text, or the cause it is
// skipped and counted under
pub type Scalar = Result<String, &'static str>;
...
// spec: gate-sdk/SPEC.md §check-action-run-path — a single-line value is a plain scalar up to its
// first ` #`, or a quoted scalar with its quotes removed; a backslash escape, and a quote the line
// never closes, are the YAML parsing the extractor declines, so each is a skip cause
pub fn scalar_text(v: &str) -> Scalar {
    let v = trim_htab(v);
    if let Some(r) = v.strip_prefix('"') {
        if r.contains('\\') {
            return Err("escaped double-quoted value");
        }
        return match r.find('"') {
            Some(end) => Ok(r[..end].to_string()),
            None => Err("quoted value not closed on its line"),
        };
    }
    if let Some(r) = v.strip_prefix('\'') {
        ...
        return Err("quoted value not closed on its line");
    }
    if v.starts_with('#') {
        return Ok(String::new());
    }
    Ok(strip_comment(v).to_string())
}
...
pub fn ghexpr(s: &str) -> Option<String> {
    let mut out = String::new();
    let mut rest = s;
...
pub struct Extractor {
    ...
    blockstart: usize,
    nblk: usize,
    pend: Vec<(Pending, usize)>,
    stepcol: i64,
    stepshell: String,
    stepwd: Option<String>,
    pub items: Vec<Item>,
...
impl Extractor {
    ...
    fn endblock(&mut self) -> Result<(), Refusal> {
        ...
        self.pend.push((Pending::Block(n), start));
        Ok(())
    }

    fn clearstep(&mut self) {
    ...
    fn flushstep(&mut self) {
        ...
        let workdir = self.stepwd.clone();
        for (p, line) in std::mem::take(&mut self.pend) {
            self.jobitems.push(self.items.len());
            let (shell, runner, workdir) = (shell.clone(), Runner::NoJob, workdir.clone());
            self.items.push(match p {
                Pending::Block(n) => Item::Block {
                    n,
                    line,
                    shell,
                    runner,
                ...
                Pending::Single(text) => Item::Single {
                    text,
                    line,
                    shell,
                    runner,
    ...
    fn flushjob(&mut self) {
        ...
        for i in std::mem::take(&mut self.jobitems) {
            let (Item::Block { shell, runner, .. } | Item::Single { shell, runner, .. }) =
                &mut self.items[i];
            if shell.is_none() {
                *runner = class;
            }
    ...
    fn processkey(&mut self, rest: &str, col: i64) -> Result<(), Refusal> {
        ...
        if key_of(rest) == Some("working-directory") {
            let v = &trim_htab(rest)["working-directory:".len()..];
            self.stepwd = Some(match scalar_text(v) {
                Ok(t) => t,
                Err(_) => trim_htab(v).to_string(),
            });
            return Ok(());
        }
        ...
        let text = scalar_text(&v).map(|t| ghexpr(&t).unwrap_or(t));
        self.pend.push((Pending::Single(text), self.fnr));
        Ok(())
    }
...
pub fn records(text: &str) -> Vec<&str> {
    if text.is_empty() {
        return Vec::new();
    }
...
pub fn extract(text: &str) -> Result<Extractor, Refusal> {
...
pub fn actions_shaped(text: &str) -> bool {
    records(text)
        .iter()
...
pub fn print_refusal(name: &str, doing: &str, file: &Path, r: &Refusal) -> i32 {
    ...
    eprintln!("refuses rather than {} a mangled fragment (fail-closed):", doing);
...
mod tests {
    ...
    fn singles(text: &str) -> Vec<(Scalar, String, Option<String>)> {
        let Ok(ex) = extract(text) else {
            panic!("the extractor refused a case that should extract")
        };
        ex.items
            ...
            .filter_map(|i| match i {
                Item::Single {
                    ...
                    workdir,
                    ..
                } => Some((text, resolved(&shell, &runner), workdir)),
                Item::Block { .. } => None,
            })
            .collect()
    }
    ...
    // spec: gate-sdk/SPEC.md §check-action-run-path — the single-line forms: a plain scalar up to
    // its first ` #`, a quoted one unquoted, and the two skip causes
    #[test]
    fn a_single_line_value_is_read_in_each_scalar_form() {
        assert_eq!(scalar_text("bash a/b.sh # why"), Ok("bash a/b.sh".to_string()));
        assert_eq!(scalar_text("\"bash a/b.sh\""), Ok("bash a/b.sh".to_string()));
        assert_eq!(scalar_text("'it''s a/b.sh'"), Ok("it's a/b.sh".to_string()));
        assert_eq!(scalar_text("\"a\\tb\""), Err("escaped double-quoted value"));
        assert_eq!(scalar_text("'open"), Err("quoted value not closed on its line"));
    }

    // spec: gate-sdk/SPEC.md §check-action-run-path — a single-line item carries its step and job
    // dialect exactly as a block does, and a step's literal `working-directory:` beside it
    #[test]
    fn a_single_line_item_carries_its_dialect_and_working_directory() {
        let yaml = "jobs:\n  j:\n    runs-on: windows-latest\n    steps:\n      - run: bash x/y.sh\n        working-directory: sub\n      - shell: bash\n        run: \"x/y.sh\"\n";
        let got = singles(yaml);
        assert_eq!(got.len(), 2, "expected two single-line items: {:?}", got.len());
        assert_eq!(got[0].0, Ok("bash x/y.sh".to_string()));
        assert_eq!(got[0].1, "!windows");
        assert_eq!(got[0].2.as_deref(), Some("sub"), "the step's working-directory was lost");
        assert_eq!(got[1].0, Ok("x/y.sh".to_string()));
        assert_eq!(got[1].1, "bash");
        assert_eq!(got[1].2, None, "a working-directory leaked into the next step");
    }
}
