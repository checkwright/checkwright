...
nav_child_order: 1
---
<!-- door-contributor: a generated projection whose only door is its own regeneration recipe, read by the contributor who regenerates it -->
# Enforcement map

_Generated by `bash gate-sdk/bin/run-gates.sh --emit enforcement-map`; do not hand-edit — `check-enforcement-fresh` byte-compares this page against the emitter._

Every governed surface in this repo is held by one enforcement class, ordered here from hardest to softest:

"#;

const MID: &str = r#"
The rows below derive from the class registries — the gate registry, the KPI registry, the harness settings hooks, the evidence-suite config, and the `# enforce:` markers a non-gate surface declares itself with — so this map cannot drift from what actually runs. A registry a consumer has not adopted leaves its section absent.
