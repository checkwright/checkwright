...
- **The opening.** *GNU-first* becomes: the engine assumes the POSIX userland
  ...
  coreutils. **Inferred, not run:** that a stock macOS host without Homebrew coreutils installs the starter profile and runs its battery clean; delta 8's macOS legs are the oracle, and no local command reaches a BSD userland.
...
- `docs/install.md` §Requirements, both remedy blocks and their paragraphs
  (delta 7).
