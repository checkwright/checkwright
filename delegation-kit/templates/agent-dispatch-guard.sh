#!/usr/bin/env bash
# spec: delegation-kit/SPEC.md §The delegation model — consumer-copy PreToolUse(Agent) dispatch-shape guard: D1 fork ban, D2 read-only isolation claim, D3 nested-dispatch advisory; register under matcher `Agent` beside the budget guard (§Layout and configuration)
set -uo pipefail
