...
- **Budget-check before *each* dispatch in a fan-out**, not once at the start.
  Run `--usage-verdict` on the gate binary `GATE_SDK_NATIVE_BIN` names (verdict
  exit 0/1/2 from `usage.txt` —
  ...
  **Do not read live budget off the statusline while parked on a dispatch.**
  ...
  mid-dispatch (a `watch -n 30` over the gate binary's `--usage-verdict` arm); this is
