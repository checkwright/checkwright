...
is the boolean knob delta 3 mints, defaulted **on**, so the producer is reachable
...
- **`check-knob-citation` and `check-knob-default-coupling`** — red conditions:
  *a knob mentioned in prose that no config defines*, and *a default stated in
  prose that differs from the code's*. **Monotone** in the knob set: delta 4 adds
  a knob **and** its SPEC roster entry **and** its default in the same unit, so
  neither reader acquires an unpaired subject. Named because a knob minted in a
  SPEC ahead of its assignment is the standard way to red both at once.
...
- **lifecycle-kit/SPEC.md §Layout and configuration** — the new boolean knob joins
  the roster with its default (delta 3).
