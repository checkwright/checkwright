...
- `printf 'pid=1 run=probe\r\n'` into a record, then `bash gate-sdk/bin/run-gates.sh --only check-producer-liveness -- <record>`: the member reports **exit 2**, `carries no readable 'pid=<n> run=<key>' record`. The front end itself exits 1, since it folds the member's exit; the binary's `check-producer-liveness <record>` returns the 2 (re-run at build). A CRLF-terminated record is corrupt under the grammar, because the run key then ends in `\r`. So a record PowerShell writes with its default line ending is unreadable, and the canonical spelling in delta 3 writes a bare LF.
...
**Applied at build, reporting.** Every round costs a push and there is no local oracle, so the step has to be right the first time. `install-smoke-sh-windows` gains a step after `guard decision table under Git Bash`: **producer liveness over Windows process ids (reporting)**, run under `shell: bash` on the artifact the normalize step placed.

The step makes a fresh record directory under `$SMOKE_TMP`, spelled `cygpath -m` so bash and the native binary read one path. For each row it reads a verdict with the gate binary's `check-producer-liveness <record>`, called directly because the `run-gates.sh --only` front end folds a member's exit 2 into 1, and a **ground truth** with `pwsh -NoProfile -Command "if (Get-Process -Id <n> -ErrorAction SilentlyContinue) { 'alive' } else { 'gone' }"`, every PowerShell call made with MSYS argument conversion off. It reads both while the process runs and again after it is stopped (`Stop-Process -Id <n>` and a wait on the ground truth reading `gone`). The rows:

- **(a) An MSYS pid.** `sleep 300 &` under Git Bash, recorded `pid=$! run=msys`. This is the shipped path, never before witnessed on this host. `Get-Process` answers only the Windows namespace, so this row's ground truth reads the Windows pid `/proc/<pid>/winpid` names, and it is stopped with `kill` and reaped with `wait`, since an unreaped MSYS child could still answer `kill -0`.
...
- **(c) A detached child.** `pwsh -NoProfile -Command '(Start-Process pwsh -ArgumentList "-NoProfile","-Command","Start-Sleep 300" -PassThru -WindowStyle Hidden).Id'`, recorded by `printf` with a bare LF. `-WindowStyle Hidden` is the spelling the leg's hosted one-liner step already launches with on this runner; the child is detached either way.
...
Each row prints `<row> truth=<alive|gone> gate=<exit>` at both readings. The step then prints one verdict line. It says `predicate answers Windows pids` when rows (b), (b′) and (c) read exit 1 while alive and exit 0 once gone, and `predicate misreads Windows pids` otherwise. When one of those rows' ground truth did not read `alive` and then `gone`, the row measured nothing, and the line says `predicate unmeasured` naming the row rather than picking a branch off it. It prints row (a)'s and row (e)'s outcomes on lines of their own; row (e) reads `gone` when the job's process is gone within 15 seconds of its session's exit, which is teardown rather than survival. It exits 0 whatever it finds.

**Measured on run 36053710927** (`install-smoke-sh-windows`): the verdict line reads `predicate misreads Windows pids`, so delta 2 takes its red branch. Rows (b), (b′) and (c) each read exit 0 while `Get-Process` reads them alive, a false free, and exit 0 once gone. Row (a)'s MSYS pid differs from its Windows pid and reads exit 1 then 0, so the shipped path holds. Delta 3's canonical write produces a bare-LF record under pwsh and under 5.1, which the gate parses. Row (d) reads exit 2 on all four writes: CRLF, and 5.1's `>` UTF-16LE with a BOM. Row (e)'s job process is gone the moment its session exits, so delta 3's conditional is not taken.
