...
|---|---|
| `check-action-gh-repo` | **Reference-only until its descriptor lands.** Its descriptor cannot ride the commit that adds its implementation — `check-gate-tamper`'s meta-path roster does not reach `native/` (its row above), so implementation and declaration land in separate commits by force rather than by choice. This row is what makes the state between them a recorded disposition instead of the stranded implementation assertion B exists to catch, and it is removed by the commit that lands the descriptor. Like the row beside it, the module meanwhile keeps the substrate exercised: it is a second real gate rule for `cargo test` and the `native_crate` evidence suite to assert over, and a second member for the read-declaration unit tests to hold their `?` against. |
