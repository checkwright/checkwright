...
|---|---|
| `check-action-gh-repo` | **Reference-only for the length of one commit.** Its descriptor cannot ride the commit that adds its implementation — `check-gate-tamper`'s meta-path roster does not reach `native/` (its row above), so implementation and declaration land separately by force rather than by choice. This row is what makes the state *between* those two commits a recorded disposition instead of the stranded implementation assertion B exists to catch, and it is removed by the commit that lands the descriptor. |
