...
# no-port: gate-sdk/SPEC.md §Consumer smoke, The port disposition — leg 1 of the class ruling of 2026-08-30, the config bridge. This library is sourced into its callers own shell and resolves GATE_SDK_NATIVE_BIN through lib/gate.sh accessor to place the binary, so it sits inside the bridge rather than beside it; the accounting the harness it builds then runs probes unregistered gates through gate_command, and §lib/gate.sh rules exactly one place a knob value is computed, so a crate-side form would be the second producer criterion 6 refuses. Its one remaining shell sourcer is outside this cut and keeps sourcing it (demo/run-demo.sh, which takes csmoke_place_binary alone rather than the builder), and the upgrade suite and the AGENTS.md smoke are both *spawn-side* callers: each bridged arm invokes these helpers in a bash that sources this unchanged library, through one shared crate-side spawn wrapper, which creates no second producer and is why both ports cleared criterion 6 on the duplication-absent road. Structural, not a sizing judgment.
