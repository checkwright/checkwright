...
# no-port: gate-sdk/SPEC.md §Consumer smoke, The port disposition — leg 1 of the class ruling of 2026-08-30, the config bridge. This library is sourced into its callers own shell and resolves GATE_SDK_NATIVE_BIN through lib/gate.sh accessor to place the binary, so it sits inside the bridge rather than beside it; the accounting the harness it builds then runs probes unregistered gates through gate_command, and §lib/gate.sh rules exactly one place a knob value is computed, so a crate-side form would be the second producer criterion 6 refuses. Its shell sourcers are outside this cut and keep sourcing it (context-kit/smoke/agents-md.sh and demo/run-demo.sh, the last of which sources it for csmoke_place_binary alone rather than for the builder), and the upgrade suite is now a *spawn-side* caller: its bridged arm invokes these helpers in a bash that sources this unchanged library, which creates no second producer and is why that port cleared criterion 6 on the duplication-absent road. Structural, not a sizing judgment.
