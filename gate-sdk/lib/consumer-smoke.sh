...
# no-port: gate-sdk/SPEC.md §Consumer smoke, The port disposition — leg 1 of the class ruling of 2026-08-30, the config bridge. This library is sourced into its callers own shell and resolves GATE_SDK_NATIVE_BIN through lib/gate.sh accessor to place the binary, so it sits inside the bridge rather than beside it; the accounting the harness it builds then runs probes unregistered gates through gate_command, and §lib/gate.sh rules exactly one place a knob value is computed, so a crate-side form would be the second producer criterion 6 refuses. Its shell sourcer set is now empty — the adoption walkthrough, which took csmoke_place_binary alone rather than the builder, was the last one and has ported — and that empties a sourcer count, never leg 1: the ground above is a property of this library (it resolves a knob through lib/gate.sh accessor), not of how many shells source it. The upgrade suite, the AGENTS.md smoke and the walkthrough are all *spawn-side* callers: each bridged arm invokes these helpers in a bash that sources this unchanged library, through one shared crate-side spawn wrapper, which creates no second producer and is why both ports cleared criterion 6 on the duplication-absent road. Structural, not a sizing judgment.
