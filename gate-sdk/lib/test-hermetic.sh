...
# spec: gate-sdk/SPEC.md §lib/test-hermetic.sh — pin every kit's <KIT>_CONFIG_FILE (and a static kit's <KIT>_KNOB_FILE) to one shared empty file so a bespoke gate-tests/*.test.sh runs on kit defaults, never the invoker's cwd config; knob-free by design (a config-pinning tool cannot be configured by the surface it pins)
...
for _th_kit in "$_th_root"/gate-sdk "$_th_root"/*-kit; do
    ...
    # spec: gate-sdk/SPEC.md §lib/test-hermetic.sh — a static kit reads its knob file under the suite's cwd, so it is pinned too; gate-sdk is skipped because its name would land in the GATE_SDK_KNOB_ bridge namespace
