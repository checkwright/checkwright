...
# allowed even under an empty knob. An empty GATE_SDK_KNOB_FILE isolates the
# test from this repo's knob file (which sets the knob). The chrome link
...
: > "$tmp/empty-config.knobs"     # neutralize the repo's gate-sdk-config.knobs
...
run() {  # run() <config> <theme-dir> <knob> -> sets rc/out
    out="$(gate_env GATE_SDK_KNOB_FILE="$1" GATE_SDK_GRAPH_THEME_DIR="$2" \
...
run "$tmp/empty-config.knobs" "$tmp/theme" ""
[[ "$rc" -eq 1 ]] \
...
run "$tmp/empty-config.knobs" "$tmp/theme" "https://third-party.example"
[[ "$rc" -eq 0 ]] \
...
run "$tmp/empty-config.knobs" "$tmp/absent" ""
[[ "$rc" -eq 0 ]] \
