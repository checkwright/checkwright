...
# rule for a kit without gate-tests, and the fail-closed missing-doc path.
#
...
# does not. Both are registered in README; the runner doc names only alpha's
# fixture line — beta owes none (no gate-tests), alpha's presence is the pass.
...
run "b-owed-satisfied" 0 "1 shipping gate-tests" "run-gate-tests.sh alpha-kit/gate-tests alpha-kit/checks"
...
echo "check-kit-registration.test.sh: clean (assertion B satisfied/missing, B-owes-nothing, fail-closed missing-doc — 3 cases)"
exit 0
