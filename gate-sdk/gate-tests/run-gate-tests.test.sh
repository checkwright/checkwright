...
#
# Both dispatch spellings are exercised, because a `.sh` member and a `.gate`
# member are spawned by different argv shapes and a one-sided fix passes either
# row alone; each reads the bare name out of the per-case child environment.
mk_writer() {
    local dir="$scratch/writer/stub-writer" bdir="$scratch/writer/stub-native"
    mkdir -p "$dir/good" "$dir/bad" "$bdir/good" "$bdir/bad"
    printf 'STUB-WRITER: clean (stub)\n' > "$dir/good/expect.txt"
    printf 'alpha fired\n' > "$dir/bad/expect.txt"
    printf 'STUB-NATIVE: clean (stub)\n' > "$bdir/good/expect.txt"
    printf 'alpha fired\n' > "$bdir/bad/expect.txt"
    { printf '#!/usr/bin/env bash\n'
...
# driven through a stand-in that runs stub-native itself and forwards every other
# argv — the arm's own included — to the real binary. That keeps the row an
# end-to-end exercise of the live arm rather than a replay of its resolver: the
# stand-in is what GATE_SDK_NATIVE_BIN names, so the runner resolves the case
# through it exactly as it resolves a real .gate member.
: >"$scratch/checks/stub-native.gate"
{ printf '#!/usr/bin/env bash\n'
  printf 'if [[ "$1" == stub-native ]]; then\n'
  # Falling back to the relative default is what makes an unpinned case
  # observable as a write inside the case dir.
  printf '    D="${GATE_SDK_TMP_DIR:-.tmp}"\n'
  printf '    mkdir -p "$D" && : >"$D/stub-native.marker"\n'
  printf '    if [[ "$PWD" == */bad ]]; then echo "alpha fired"; echo "  help: stub remedy"; exit 1; fi\n'
  printf '    echo "STUB-NATIVE: clean (stub)"; exit 0\n'
  printf 'fi\n'
  printf 'exec "%s" "$@"\n' "$GATE_SDK_NATIVE_BIN"
} > "$scratch/nativebin"
chmod +x "$scratch/nativebin"
t="$(mk_writer)"
...
out="$( cd "$scratch" && unset GATE_SDK_TMP_DIR \
    && GATE_SDK_NATIVE_BIN="$scratch/nativebin" gate_arm_run --run-gate-tests "$t" "$scratch/checks" 2>&1 )"; rc=$?
assert_rc  writer "$rc" 0
for marker in stub-writer stub-native; do
...
for case_dir in "$t"/stub-writer/*/ "$t"/stub-native/*/; do
    [[ -e "$case_dir/.tmp" ]] && {
