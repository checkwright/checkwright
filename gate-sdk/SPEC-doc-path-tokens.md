...
| `.github/workflows/*.yml`, `*.yaml`, `.github/ISSUE_TEMPLATE/*.yml`, `.github/workflows/publish.yml` | 5, 4, 4, 1 | platform-fixed |
| `.gitignore`, `.gitattributes`, `SECURITY.md` | 3, 1, 1 | platform-fixed |
| `scripts/git-hooks/*`, `scripts/graph-theme/*`, `scripts/gate-tests/*`, `scripts/settings-pins.conf`, `scripts/msg-patterns.list` and the other named `scripts/` files | 1 each, except the two named `.conf`/`.list` files at 2 each | dir or file knob |
...
descriptors (`git ls-files 'scripts/*.gate' | grep -v /gate-tests/`; the bare glob
without that exclusion also matches five fixtures nested under
`scripts/gate-tests/`, for 22).
...
after the other: each edits its own tokens in the `couples=`/`trigger=` comma
list — this delta converting a consumer-path literal to a `knob:` token, that one
dropping a crate-file token the derivation now supplies — and the align audit
confirmed the two classes never name the same token on one descriptor, so
neither edit's list-splice depends on the other having landed first.
...
- Every kit-shipped `.gate` descriptor carrying a surveyed literal, under
  ...
  `lifecycle-kit/checks/`, `queue-kit/checks/` and `site-kit/checks/` (delta 3).
  guard-kit is not in this list: its one descriptor,
  `guard-kit/checks/check-door-binding.gate`, carries only `kit:`, crate and
  `knob:` tokens, so it has no surveyed literal to convert.
