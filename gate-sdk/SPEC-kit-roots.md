...
- **gate-sdk/SPEC.md §The first cohort, and the rule that selects the next** — a
  ...
  `spec_manifest_files` cohort has: the shared derivation (delta 1), the
  criterion-4 finding on the fifth member (delta 2), and the library move
  delta (3) bought. Coordinate with `SPEC-port-criteria.md` delta (3), which
  edits the same section's `check-roadmap-fresh` bullet.
...
- **gate-sdk/SPEC.md §Meta-gate conservation for the binary substrate** — nothing
  ...
  amendment records that verification as the delta (delta 2) so a merge does not
  quietly edit either.
...
- **gate-sdk/SPEC.md §check-kit-registration, §check-smoke-entry-guard,
  ...
  siblings carry (delta 4); no invariant and no calibration changes, because a
  port that changed a rule would have no parity to prove.
- **`scripts/gates.list`, `gate-sdk/README.md`'s roster, the generated pre-commit
  ...
  are generated projections or rosters the port (delta 4) stales; each is
  regenerated by the command `docs/site-architecture.md` §Generated projections
  rosters for it, not hand-edited.
