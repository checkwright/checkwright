...
**inventory-command** — `DRIFT_KIT_KIT_ROOTS=gate-sdk bash drift-kit/templates/kpi-deprecated-surface.sh`, the live-marker scan over the `CANON_KIT_DEPRECATION_MARKERS` roster (`scripts/canon-config.knobs`) — the same resolution `check-deprecation-task` enforces between majors. An empty roster reports `n/a`: nothing to disposition.
