...
## 2026-08-23 build — Which surfaces in this tree carry the mandated in-turn-wait rule's ordering and its loop spelling?
- corpus: every tracked .md, .sh, .rs, .list and .txt outside .git and .tmp
- oracle: grep -rn "event-stream|stays armed|condition goes true|duration expires" over the tree, stderr unsilenced
- rev: dfa3fb7f839762aa34532c6cf4e6dd3ad76a426c
- finding: Eight carriers, not the four the retired amendment's roster named. Owner: delegation-kit/templates/agent-execution.md (Which primitive + the liveness clause). Restatements: .claude/agents/stage-session.md, .claude/agents/audit-sweep.md, guard-kit/lib/guard.sh rule 13's block message, guard-kit/SPEC.md rule 13's corrective text AND a second guard-kit/SPEC.md site recording what naming a harness's forms costs. Projection: docs/guard-kit/SPEC.md (regenerated, never edited). Transient: the TASK-QUEUE.md entry itself. NOT a carrier, contrary to the amendment: delegation-kit/SPEC.md returns ZERO hits for any of the four phrasings -- Operative residency carries the residency sanction and the propagation record, never a copy of the rule. Nor is CLAUDE.md, whose Agent execution line is a pointer. The roster undercounted because it was assembled from the section's own list of noticed carriers; the section's own paragraph already says N is discovered by grepping the phrasing rather than by consulting the roster, and this is the second firing of that.
