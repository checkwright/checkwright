...
## 2026-08-26 align — Do the eight windows-adopter-unblock amendments' tree-state and cross-component claims hold against the live tree, and are their {mechanical}/{design-bearing} delta tags accurate
- corpus: the 8 amendment files plus every file/section each names as an update target or a factual claim (gate-sdk, lifecycle-kit, queue-kit x2, drift-kit, evidence-kit, delegation-kit)
- oracle: manual re-read of each amendment against canon-kit's amendment-lifecycle rules (delta grammar, existing-sections-updated roster completeness) plus three dispatched read-only isolation:worktree audit-sweep agents that re-verified every tree/prose claim by direct grep and file read
- rev: 9f620d135f6e49dc058f022ae307773f980a97fe
- finding: 27 of 31 checked claims PASS verbatim; 4 real inaccuracies found and corrected in place (evidence-kit/SPEC-baseline-grain.md delta 4's arm-name undercount 10-vs-12, lifecycle-kit/SPEC-ruling-authority.md's SHA-citation overclaim against gate-sdk/SPEC.md, queue-kit/SPEC-retired-citations.md's close-already-runs-a-tool overclaim, delegation-kit/SPEC-worktree-reader.md's writes-only mischaracterization of a sibling queue entry's scope); one pre-existing TASK-QUEUE.md prose inaccuracy gap-filed rather than fixed inline; no delta mis-tagged with high confidence, one borderline case (baseline-grain delta 3, tagged mechanical, carries a real cost-tradeoff design refusal) flagged to the lead rather than retagged unilaterally
