...
## 2026-08-12 align — Does gate-sdk's bin/ argv-shape census (SPEC-bin-argv.md, five members) miss a member sharing the same arity-only-not-shape defect?
- corpus: every kit's bin/*.sh, re-verified against SPEC-bin-argv.md delta 2's table
- oracle: grep each bin/ tool for its arity check ($#) and its -h/--help handling; cross-check gate-sdk/bin/run-gate-tests.sh specifically
- rev: 045a349788ff8b2f6e63a07c8f5302e3de5b1397
- finding: Confirmed the five-member table (file-gap.sh, kfric.sh, file-survey.sh, cite-survey.sh, enter-stage.sh) accurate for what it covers, but gate-sdk/bin/run-gate-tests.sh is a SIXTH candidate the census missed: TESTS_DIR="${1:-...}" and GATE_DIRS=("${@:2}") are free-text directory paths gated only by $# -gt 1 (no membership/shape check) with zero -h/--help handling anywhere in the file. Lower severity than the five (it does not capture onto a durable governed surface -- a bad path fails a -d existence check with a misleading 'no fixture tree at --help' message rather than corrupting state), but it fits the amendment's own stated rule scope ('binds on every free-text tool'). doctrine-kit/bin/install-doctrine.sh, doctrine-kit/bin/stage-rules.sh and evidence-kit/bin/diff-baseline.sh were also checked and are correctly non-members (verb-shaped dispatch or membership/existence-validated already). Escalated rather than folded into the amendment, since adding a sixth member widens the amendment's asserted deliverable.
