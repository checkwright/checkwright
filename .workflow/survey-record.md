...
## 2026-08-14 build — Applying the eighth cohort's selection procedure to the repaired --group partition, which group does it actually yield?
- corpus: the 64 still-shell members of scripts/gates.list at 131d6b8a, plus gate-sdk/SPEC.md's standing port rulings
- oracle: bash gate-sdk/bin/port-blockers.sh --group, read against gate-sdk/SPEC.md 'Meta-gate conservation for the binary substrate' and 'The port-candidate criteria'
- rev: 5d5b731bc2b895185d2658c61ee4bbe3079b9532
- finding: Run at 131d6b8a: 104 scanned, 48 groups, ZERO undecidable, 40 already ported. Supersedes the 103/47/0/40 block above (check-settings-paths landed since). The procedure does NOT yield the candidate that block named. Group 1 (14, libs=fail_closed globs=-) is not a cohort, operator-ruled: fail_closed derives no corpus. Group 2 (check-install-disposition + check-readme-roster) is NOT takeable whole: gate-sdk/SPEC.md 'Meta-gate conservation' rules check-install-disposition stays a shell gate, because its assertion is about the declaration-and-dispatch relation and a compiled form could pass itself with a broken binary; the section states that distinction is 'exact rather than a judgment call'. It is a member the port does not take, like check-crate-arms, which no --group column reports. Group 2 therefore reduces to one takeable member. Of the remaining 2-member groups, group 3 has a ruby blocker (criterion 7) and group 4 (check-spec-dod-singleton + check-spec-derivable-section, libs=fail_closed,spec_canonical_specs globs=-) fails only criterion 3 (align-only). Since 'The first cohort' sequences criterion-4-and-7 failures LAST, a group failing only criterion 3 outranks one failing 4, and group 4 is also the larger. Yield: group 4, landed as the eighth cohort at 5d5b731b. Transferable: the tool's columns are not the whole screen — a member spec-held on shell is invisible to c2/c3/c7, so the standing rulings must be read before a group is taken.
