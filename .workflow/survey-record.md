...
- edges: platform-support-ci-matrix 10, prose-filename-citation-liveness 6, powershell-installer-surface 5, overlay-only-oracle-grants-uncommitted 4, then a band at 3 (unqualified-section-citation-liveness, threshold-recurrence-routing-residency, session-model-identity-verification, scratch-citation-skill-surface-reach, retired-slug-live-pointer-citation, queue-entry-grammar-single-owner, guard-steer-grant-mismatch, dispatch-claim-evidentiary-tier-unmarked, companion-toolkit-profile, build-stage-tier-economics, benchmark-ab-experiment) and roughly forty at 2; the citation-liveness family sums to about 18 across low-ranking members. Two deltas since 2026-08-28: powershell-installer-surface 6 -> 5, native-gate-port-remaining-corpus 3 -> 2.
...
## 2026-08-29 build — Which tracked surfaces name the drift-kit shell collator or its kpis/ plugin files, and would go stale when the 2026-08-29 port deletes them?
- corpus: the whole tracked tree: git grep over drift-report, kpis/, kpi-, drift-kit/bin, CONTEXT_KIT_DRIFT_REPORT, plus targeted reads of scripts/, .claude/, docs/, every SPEC.md and README.md, installer/, demo/, native/src/
- oracle: git grep -n 'drift-report' && git grep -n 'drift-kit/kpis/' -- ':!docs/'
- rev: 48cff8d3ffd3937c4fde92c188ac9becb5f5179f
- edges: none
- finding: The amendment's own 'Existing sections updated' roster was INCOMPLETE by six spots, every one found and swept in the same commit: gate-sdk/SPEC.md §The path-dialect contract's worked disposition (written hours earlier by batch 1, and citing the very file this cut deletes), its checked-index-mode worked example, and §The port-candidate criteria's 'one live registry, drift-kit/kpis/' claim in BOTH cohort sections; context-kit/SPEC.md L456 and L1159; drift-kit/README.md's two-tier resolution wording. Also confirmed: the four MUST-CHANGE classes the amendment did name (settings grants, both session-context.sh copies, context-kit's knob prose, the two READMEs). CLASSIFICATION RULE THAT MADE THE SWEEP TRACTABLE: a kpi-<name> mention is a REGISTRY NAME and survives the port; only a kpis/<name>.sh or bin/drift-report.sh spelling is a file path that dies. Dated docs/posts/ release announcements were deliberately left stale: a post records what shipped at its date and is not a live pointer.

## 2026-08-29 close — How saturated is the per-entry line cap across the live deferred pool, and does ruling density predict saturation?
- corpus: TASK-QUEUE.md ## Deferred, all 270 entries, extent measured per slug
- oracle: bash gate-sdk/bin/run-gates.sh --emit queue-index --extent <slug>, looped over every deferred lead line
- rev: 59fa4f91741eb91cf6d94be3eb5d4bec6ec39f9d
- edges: none
- finding: SATURATED BAND IS 17 OF 270. Eight entries sit at 51 physical / 50 counted lines, the cap exactly (ruling-record-condition-staleness-probe, native-gate-port-remaining-corpus, lead-specifies-constraint-not-mechanism, guard-read-steer-tool-coverage, dispatch-claim-evidentiary-tier-unmarked, cardinal-notation-splits-gate-reach, bridged-knob-case-tmp-dir-override-inert, align-checklist-fanout-calibration); nine more at 50. The band is populated by the most-ruled and most-contended entries in the file, which is the correlation ruling-accretion-outgrows-the-entry-cap was filed on. TWO LIVE HITS THIS ITERATION, both first-hand: build's demotion landed native-gate-port-remaining-corpus at 51/50 with no line left for the next ruling, and this close's recurrence judgment on batch-split-stamp-ownership (also 50/50) cost six lines of compression before a five-line datum would fit. WITNESS FOR A LATER STAGE: re-run the loop; the counted figure is the extent less at most one line per declaration grammar, so an entry gaining a recurrence: or ruled: line reads 51 physical and 50 counted and is still legal.

## 2026-08-29 close — Does the shape 'a test assertion that passes because its subject errored out' occur anywhere in the shell test corpus beyond the one attested instance?
- corpus: 118 shell test and assertion files: every kit's gate-tests/ and smoke/, demo/run-demo.sh, and gate-sdk/bin/upgrade-smoke.sh, run-consumer-smoke.sh, run-gate-tests.sh
- oracle: delegated worktree-isolated sweep in six parallel groups; per file, captured-output and exit-code handling traced against five vulnerable shapes, every kit-tool and native-gate call site checked against its real argument parser, and the native binary built so subjects could be run live rather than reasoned about
- rev: 4a9a79646d4082eed14d821083adaf7f11607370
- edges: none
- finding: CLEAN AT 118 FILES: the shape occurs exactly once in this repository, the attested lifecycle-kit/smoke/install.sh arity mismatch against file-survey.sh, and that instance is already fixed at HEAD (line 266 now asserts rc equal to 2 against the five-positional signature, re-verified live). Negative assertions over captured output all sit behind a checked exit code on the same invocation; the tree's standard idiom captures status into rc under a bracketed set +e rather than discarding it; every arity and flag check matched its parser. THREE SUSPICIONS, none demonstrated exploitable: a battery-success grep that would also match a zero-gate line but is unreachable with an empty roster; check-graph-theme's bare-emission case asserted only on absent markers, a hardening gap only; and one discarded git merge exit code with no constructible false-clean path. BYPRODUCTS, both real and both filed as entries: run-consumer-smoke.sh with a single kit argument fails for every kit but lifecycle-kit on an unresolvable cross-kit knob, and upgrade-smoke.sh refuses inside any linked worktree. WITNESS: the corpus is stable, so a later stage re-buying this owes only the files added since this rev.
