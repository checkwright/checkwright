...
## 2026-08-25 spec — Which steps of the install path must precede the binary invoke, and which are conditional install logic that can move behind it?
- corpus: installer/lib/*.sh installer/bin/checkwright.sh installer/README.md gate-sdk/SPEC.md TRAJECTORY.md
- oracle: wc -l installer/lib/*.sh installer/bin/checkwright.sh; grep -n '^# install:' gate-sdk/checks/check-graph.gate
- rev: 6aff9eabdc5869367858cbfe2a70c801800fc84f
- finding: FIVE bootstrap steps and no more: resolve the payload dir, resolve the host to one Rust triple, read the target roster and resolve the artifact+sidecar pair, verify the SHA-256, exec the artifact in place. Everything else in init.sh (1249 non-smoke installer lines total; init.sh 428, doctor 164, uninstall 214, diff 66, update 34, common/ 290, dispatcher 53) is conditional install logic and moves behind the invoke; the jq preflight RETIRES rather than moving, since nothing behind the invoke reads JSON with jq. THREE non-obvious results. (1) gen-pre-commit.sh --write is NOT a third disposition class: it does not port (gate-sdk/SPEC.md §gen-pre-commit, operator-ratified 2026-08-21, criterion 6 single-producer) and check-graph is install: zero-config so a fresh consumer's hook must exist at install — but the compiled substrate ALREADY spawns bash on this exact generator from check-graph assertion D, criterion 7 clears it because bash is a consumer-audience member of context-kit/lib/toolfloor.sh's PROBE_SET, so the step moves behind the invoke as a declared bash spawn and the bootstrap itself spawns nothing. (2) THE BINDING CONSTRAINT is not any step's portability but criterion 5's omit-and-declare branch: on a host the payload carries no artifact for, or with no SHA-256 hasher, there is no binary to invoke at all, so a relocated step is unreachable and the failure mode is NO INSTALL rather than a smaller battery. A step is takeable now only if it already runs only when an artifact was selected; relocating the unconditional remainder is sequenced behind the target roster covering every supported platform, and digest-unverifiable must become a refusal at that same moment. (3) init.sh:277-308 (artifact placement + gate-sdk-config.sh seam) is the ONLY block satisfying the takeability rule outright, which is what selects it as the first cut.
