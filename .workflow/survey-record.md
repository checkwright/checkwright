...
## 2026-08-30 close — Of the queue's retired inbound citations, how many are genuine live-pointer defects needing inline correction, and how many are legitimate provenance?
- corpus: TASK-QUEUE.md's full retired-citation block: 77 retired slugs across 167 individual citation lines, as reported by queue-edges.sh
- oracle: bash queue-kit/bin/queue-edges.sh TASK-QUEUE.md, reading every header ending '(N inbound, retired)'; each candidate then read as a FULL SENTENCE in TASK-QUEUE.md rather than judged on the truncated one-line excerpt
- rev: b17dd45b8982cc853796853b2b72eed969ee676f
- edges: Classification boundary is (A) a directional pointer treating the retired slug as pending/owning/blocking work a reader could still go to, versus (B) provenance or disambiguation - DISTINCT from, split from, superseded, already marked retired - versus (C) genuinely undecidable from the entry alone. Conservative rule applied: when in doubt (C), never (A). Known blind spot: the census counts CITATIONS, not entries, so one bad entry citing three retired slugs reads as three lines. Also note the population is structurally large and benign because clearing Done retires every completed slug, so raw retired-citation count is NOT a defect signal.
- finding: A=2, B=164, C=1 out of 167. The class is a handful of defects, NOT a systemic backlog (164:2). Both (A) defects shared one narrow failure shape: a still-open design-pending entry asserting present-tense joint ownership or ruling-pendency over a slug whose disposition later changed, never revisited afterwards - expected-permission-mode-undeclared pointing at guard-grant-review (split 2026-08-22, open question moved to breadth-declaration-committed-glob-home), and recurrence-resolver-literal-match-only instructing a scope that three entries cost together when both siblings had shipped 2026-08-08 and 2026-08-17. BOTH CORRECTED INLINE at close 2026-08-30, so a re-run should return A=0 unless new instances accrued. The single (C) is state-representation-integrity's cost-them-together toward native-gate-binary-port, which needs a spec-stage read of whether that port's shipped parser discharges the typed-model half. CONSEQUENCE FOR A FUTURE GATE: a broad 'cites a retired slug' flag would be ~99 percent false positive here; any gate for this class needs the narrow heuristic instead, an open entry asserting present-tense joint ownership or pendency over a slug absent from the live entry set. DO NOT RE-BUY the full 167-line sweep without first re-running queue-edges.sh and diffing the retired set against this rev.
