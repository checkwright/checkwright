...
## 2026-08-24 build — How often does a commit change the tracked non-test shell corpus the --tree arm walks, and how often does such a commit already owe a generated-hook regeneration for an unrelated reason?
- corpus: the last 200 commits reachable from HEAD
- oracle: for each commit, git show --name-status filtered to A/D of *.sh minus *.test.sh minus gate_path_pruned; cross-tabulated against whether the same commit touched a .gate or scripts/gates.list
- rev: bb7970bd6d587a6b05ea9a9abb8547c13718b16d
- finding: 13 of 200 commits changed the corpus file set, and 10 of those 13 also touched a .gate or gates.list and so already owed a hook regeneration. MARGINAL cost of a whole-tree-derived measured key riding the hook's baked knob values: 3 commits in 200, about 1.5 percent. This was bought to settle whether the new tree-shell-owed key is affordable, against a dispatched sweep's prediction that it would be 'a routine event' and 'the finding to weigh hardest'. The measurement is what refuted it. The same number is the input to any future decision about binding a <!-- measured: --> marker to that key: the marker would add a governed-prose edit to all 13 rather than to 3, which is the reason no marker was written.

## 2026-08-24 build — Which code paths in the gate crate spawn a program other than git, and which of those sites are production rather than test-only?
- corpus: every spawn site in native/src — proc::run / run_merged / run_with_stdin / run_streamed plus any direct Command::new — classified by whether it sits under a #[cfg(test)] region, counting the inner #![cfg(test)] module-attribute form
- oracle: grep -rn 'Command::new("|proc::run("|run_merged("|run_with_stdin("|run_streamed(' native/src/ | grep -v '"git"' — then open each file to classify, because a depth-tracking heuristic over the outer #[cfg(test)] form alone misreads a module carrying #![cfg(test)] as production
- rev: d6674825fa63f6db73c2fce94d0c8d6b57e5946d
- finding: PRODUCTION, on the shipped gate path: bash (gates/graph.rs:376, the hook generator); bash and ps (evidence.rs:136,146, check-producer-liveness); shellcheck (gates/shellcheck.rs:77 and gates/action_run_shell.rs:362); cargo and rustc (gates/crate_arms.rs:29,53); the renderer knob's command (gates/docs_render_fidelity.rs:45); and date (emit/queue_index.rs:396, a non-gate emit arm the seven *-fresh members re-run). TEST-ONLY: ere.rs:655 (awk), main.rs:371, walk.rs:596 and :632, shellcheck.rs:141, and the whole of gates/settings_pins_parity.rs, which carries #![cfg(test)] as an inner attribute and so is NOT the undeclared jq dependency a naive scan reports it as. Consequence: check-graph's bash was the first non-git spawn on the shipped gate path, never the first in the crate.

## 2026-08-24 close — Which shell declaration-path literals in governed prose no longer resolve, and does each clear on inspection or is it a live pendency finding?
- corpus: every tracked *.md outside docs/posts/ (336 files), path literals matching <kit>/{checks,bin,lib}/*.sh
- oracle: git grep -ohE '[A-Za-z0-9_.-]+/(checks|bin|lib)/[A-Za-z0-9_.-]+[.]sh' -- '*.md' ':!docs/posts/*' | sort -u, then existence-test each against the worktree
- rev: a5c82f681bf6a0c150717590c12fda1dcff636f0
- finding: 66 unique literals, 16 non-resolving, ZERO live findings. The 16 classify as: 4 wrong-path examples INSIDE the filed gap kit-bin-entry-point-unrostered (canon-kit/bin/md-section.sh, gate-sdk/bin/md-section.sh, gate-sdk/bin/section.sh, lifecycle-kit/bin/drain-stage.sh); 2 never-committed ad-hoc parity probes named in past tense (canon-kit and evidence-kit zz-parity-probe.sh, git log --all empty for both); 3 past-tense narrative of an EARLIER port's deletions (drift-kit/bin/trajectory.sh, queue-kit/bin/roadmap.sh, and evidence-kit/checks/check-producer-liveness.sh whose delegation-kit/SPEC.md citation correctly explains why the default was removed); 4 deliberate placeholders or hypothetical examples (evil/checks/check-x.sh, gate-sdk/bin/x.sh, gate-sdk/checks/check-x.sh, package/bin/widget.sh); 1 correct non-repo path (package/bin/checkwright.sh, the extracted tarball payload prefix, cited at docs/install.md:220 and installer/README.md:36); 1 deliberately-broken fixture (spec-kit/lib/spec.sh); 1 design-pending future deliverable (queue-kit/bin/queue-edit.sh). WHY A LATER STAGE WANTS THIS RATHER THAN RE-RUNNING IT: the classification is the expensive half and it is stable — 12 of the 16 are structurally permanent non-resolvers (placeholders, fixtures, tarball prefixes, past-tense records) that will re-appear in every future run of this census and each cost a read to clear. Re-run the oracle, diff the literal set against the 66, and only NEW non-resolvers need classifying. TAKEN AT: the close of shell-gate-tail-port-and-completion-oracle, immediately after a port deleted 8 shell gate declarations, which is the condition that makes this census worth buying at all.
