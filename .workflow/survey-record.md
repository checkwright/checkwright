...
## 2026-08-25 build — Which owed scripts/ shell rows hold this repo's private rule content, so the provenance-seam # no-port: cause reaches them, and which hold only layout and stay owed?
- corpus: the 25 owed scripts/*.sh rows reported by port-blockers.sh --tree, read header by header
- oracle: bash gate-sdk/bin/port-blockers.sh --tree | grep '^scripts/'
- rev: bd3f7d960bc3f047ab9d2a78ebe52cbe9deb5115
- finding: 10 declare, 15 owed — the inverse of the filename-shaped 15/10 (ten *-config.sh plus five vocabulary emitters against ten others). Discriminator: CLAUDE.md's convention that every kit takes its knobs with THIS REPO'S LAYOUT as the defaults, so layout inside a kit is sanctioned and only the five content classes the provenance seam names (term lists, coupling vocabularies, glossary bodies, wire-contract couplings, product constant sets) bar a port. Declared: canon-config, claim-classes, gate-sdk-config, graph-vocab, guard-config, install-transports, lifecycle-config, payload-claims, queue-config, site-config. Left owed with cause: enum-sets (derives every member from the tree; its private array lives in queue-config), context-config / delegation-config / drift-config / evidence-config (layout only; drift-config's stage roster is derived, not held), bash-guard (ambiguous — project block rules are guidance prose, none of the five classes), plus nine other mechanism rows. Over-declaring is invisible and expensive, so ambiguity resolves to owed.
