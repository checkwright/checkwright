...
- **Each member's satisfying value** — the views each of the 27 rules reads, derived from `lib/guard.sh`'s bodies with helper calls followed and other rules' predicates left out — is the table below. A disagreement with today's SPEC text is marked, and the table is what the port matches.
  ...
  | `abs_script` | `raw`, `sq dq hd`, `dequoted` | `grant_path_slot`, `worktree_confinement` | no clause |
  | `abs_prefix` | `sq dq hd` | — | no clause |
  | `expansion` | `sq hdq`, `sq dq hdq` | — | agrees |
  | `brace_glyph` | `raw`, `sq dq hd`, `dequoted` | `grant_path_slot`, `worktree_confinement` | `sq dq hd`; omits `dequoted` |
...
- **Mechanics ruled at build, to merge with this delta's prose** (the code and `consumer-rules.test.sh` already hold each):
  - The member reads the consumer knob first on every payload that parses, and a refused read blocks, as delta 3's knob posture does. Stdout that is only whitespace counts as empty. A fault-carrying envelope adds `additionalContext` after every key the library printed. A generic advise carries the fault appended to its own context, on a new line. A call the ruleset leaves undecided still logs its fall-through line under a fault, because a fault costs the consumer's rules and nothing else.
  - `view` takes the view spellings the reader carries: `raw`, `sq`, `hdq`, `sq hdq`, `sq dq hd`, `sq dq hdq` and `dequoted`. It accepts the spelling as one operand or as separate words. `body` and any other spelling are a usage error, exit 2. The command is read as the member reads it, with trailing newlines stripped, and an empty command prints nothing.
  - A child that exits without reading all of its stdin still ran. The broken pipe is not a failure to start the command, so its exit status decides.
  - This repo's `scripts/guard-rules.sh` finds the binary the way a harness-integration arm does, with gate-sdk's `gate_harness_bin`. So from a linked worktree it reaches the main checkout's binary, as the copied guard did. It prefixes its blocks with `guard-rules:`.
