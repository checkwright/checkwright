...
**The carry-forward runs after `init` has written everything it writes, and the
generated projections are what make that ordering load-bearing.** The rule picks
up every prior path `init` has not written *this* run, so anything regenerated
after it would be recorded as carried, excluded from the staged set, and left
dirty by the one commit that was supposed to contain it — `init` would report a
commit it did not make. The hook, the graph artifact and the seeded `gates.list`
are regenerated on every run, so they must be recorded *before* the carry-forward
sees them. This is invisible on an ordinary hop, where the regenerated bytes are
identical and nothing is dirty; it surfaces on a hop that changes what the
projections say — an **artifact-free upgrade of a tree whose previous payload
carried an artifact**, where the omit-and-declare set moves and every projection
derived from it is rewritten.
