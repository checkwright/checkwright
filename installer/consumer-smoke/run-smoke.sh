...
# spec: installer/README.md §The consumer smoke — the line count is asserted on the element counts and NOT by the comparison below, which cannot carry it: `${arr[*]}` joins on IFS, so two arrays whose elements differ in where the line boundaries fall can join to one identical string. No witness says `jq` could produce that aliasing against a crate-written file, so this is belt-and-braces rather than a defect repair — but the comparison's own reach is what the declaration beside it must claim, and a count is the cheapest way to make the claim true rather than narrow it
