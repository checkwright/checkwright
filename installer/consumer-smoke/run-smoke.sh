...
# spec: installer/SPEC.md §The consumer smoke — INSTALLER_SMOKE_TMP_DIR is the scratch knob; everything the smoke writes lands under it, so a run leaves the worktree untouched
...
# spec: installer/SPEC.md §The consumer smoke — pack refuses a roster target no leg built, so a host build satisfies --artifacts only while the roster is this host alone; the moment it declares a second target the smoke blocks here naming its own remedy rather than packing a payload with a hole in it
...
if [[ -n "$PREBUILT_DIR" ]]; then
    ...
    # spec: installer/SPEC.md §The consumer smoke — the binary is built rather than fabricated with a matching digest: a stand-in would drive the same placement code while leaving the one thing most likely to break — the real build's digest agreeing with what init verifies before writing — covered by nothing
...
# spec: installer/SPEC.md §The consumer smoke — --root "$REPO" is what makes the packed tree and the asserted tree the same tree by construction: $REPO is script-path-derived, so without it the current directory selects what gets packed and a run from a second checkout greens while asserting nothing about the tree under test
# spec: installer/SPEC.md §The consumer smoke — the cd is the second half of that same pinning, and it is owed at every one of these five call sites: the front-end resolves the gate binary, and runs it to read its knob files, at the git toplevel of the CURRENT DIRECTORY, so --root alone now selects only which tree is packed and the cwd selects whose tooling runs
...
# spec: installer/SPEC.md §The consumer smoke — the install is from the packed tarball with --offline, which is what proves the claim the install page makes: a one-shot vendoring installer resolves nothing from a registry, so the payload must already be inside the tarball
...
# spec: installer/SPEC.md §Profiles — the invariant is asserted against the installed payload rather than the source tree, so it holds for what an adopter actually receives
...
# spec: installer/SPEC.md §Profiles — assertion 1: every named kit resolves in the payload, which is what makes a roster a roster rather than a wish
...
manifest_report() {   # $1 = profile, $2 = consumer dir, $3 = its manifest, $4 = mismatch count, $5 = checked count, $6 = the malformed witness tuple, empty where no disagreement failed the shape test, $7 = how many disagreements failed it, $8 = the first raw line of the .files stream, $9 = the raw line of the first disagreeing entry, empty where no hash disagreement was found, $10.. = one '<path><TAB><the want the loop held><TAB><the got the loop held>' per hash disagreement, in the order the loop found them
    ...
    # spec: installer/SPEC.md §The consumer smoke — the verdict's own row joins the sample set last, deduplicated ON THE WHOLE TUPLE, so no run can exit 2 on an entry whose BYTES this report did not print; the coincidence claim is a check this report performs and prints in three outcomes, because a path-equal pair whose bytes differ is two decompositions of one recorded entry disagreeing — a finding about this harness — and collapsing it into the coincidence sentence asserts an identity nothing checked
    if [[ -z "$mal_first" ]]; then
...
assert_followups() {   # $1 = profile, $2 = the consumer init just wrote, $3 = init's captured output
    ...
    # spec: installer/SPEC.md §init — zero commands extracted is a red and never a skip: an assertion over an empty set passes vacuously, so a reflowed banner would otherwise turn full coverage into silent zero coverage, and a block this arm cannot read at all is a failure of its own construction rather than a finding about the payload
    [[ ${#cmds[@]} -gt 0 ]] \
        || { printf '%s\n' "$out" >&2; blocked "$profile: init printed no follow-up block matching the grammar installer/SPEC.md §init states, so this arm has nothing to assert over."; }
    ...
    for line in "${cmds[@]}"; do
        ...
        for (( i = ti + 1; i < ${#toks[@]}; i++ )); do
            flag="${toks[$i]}"
            [[ "$flag" == -* ]] || continue
            # spec: installer/SPEC.md §The consumer smoke — the negative control is what keeps the flag assertion from passing vacuously: it establishes that this target refuses an unknown flag BY NAME, and the refusal it prints is what the positive probe is then measured against, so no refusal string is spelled here
...
assert_install() {   # $1 = profile, $2 = scratch consumer dir
    ...
    # spec: installer/SPEC.md §init — the arm rides THIS init invocation and never the idempotent re-run below, whose no-op branch prints no follow-up block at all: an arm placed there would assert over an empty block on every profile and pass by vacuity, which is the hole the emptiness assertion exists to close arriving through the back door
    ...
    for k in ${resolved_kits[@]+"${resolved_kits[@]}"} "${lock_kits[@]}"; do
        [[ "$k" != "$reserved" ]] \
            || fail "$profile: '$reserved' is the payload's reserved artifact directory, and it is being carried as a kit root — a declared root takes no disk predicate, so every kit-root sweep would reach it"
    done
    say "kit roots: the battery resolves all ${#resolved_kits[@]} kit(s) the manifest records, and the reserved '$reserved' directory is in neither set"
    ...
    # spec: installer/SPEC.md §init — idempotence is a property of the tree, so the assertion is on the tree object and not on what the re-run printed
    ...
    # spec: installer/SPEC.md §The gate binary — selection keeps three outcomes and only ONE of them proceeds, so an install that reached this line placed a verified artifact by construction: the branch that recorded no artifact and declared its omissions is not narrowed here, it is unreachable, because the bootstrap refuses before any verb runs. The three outcomes are still asserted, in the artifact arm, where the two refusals are driven directly and told apart by message and remedy
    target="$(jq -r '.artifact.target // ""' "$LOCK")"
    seam="$(lock_own_file "$LOCK" "$GATES_DIR/gate-sdk-config.knobs")"
    list="$(lock_own_file "$LOCK" "$GATES_DIR/gates.list")"
    [[ -n "$list" ]] || fail "$profile: the manifest records no gates.list"
    # spec: installer/SPEC.md §Profiles — the registry this install wrote, kept for the monotonicity assertion the profile-invariant arm deferred: comment and blank lines dropped, so what is compared is the live membership an adopter of this profile actually receives
    ...
    # spec: installer/SPEC.md §The manifest — the binary and the seam are files init wrote, so both are on the roster it records: a path init created and did not record reads as "never installed" next run, which is the reading that lets the following install write straight through it
...
assert_value() {   # $1 = profile, $2 = scratch consumer dir -> sets VALUE_VERDICT to 'red' or 'green' for the defect
    ...
    # spec: installer/SPEC.md §The consumer smoke — the fix is the link and never the corpus
...
# spec: installer/SPEC.md §The consumer smoke — one encoding of the reversal, run on the same consumer assert_install just finished with, so both transports prove it and the masked arm proves diff and uninstall are Node-free at no extra pack cost. The tree-object equality against the pre-init seed is the load-bearing assertion and it proves more than uninstall: no other arm asserts that the roster covers everything init wrote — the per-profile check runs the other direction, entry against tree — so a file init wrote and failed to record survives the removal and reds here
assert_reversal() {   # $1 = profile, $2 = scratch consumer dir, $3 = the consumer's tree object before init ran
    ...
    # spec: installer/SPEC.md §The consumer smoke — update's --dry-run is held to the same rule over a consumer the real run would change: a committed deletion of one recorded file makes the plan name it pending, so a flag that parsed and then wrote would restore the file and move the tree object
    local head victim
    head="$(git -C "$C" rev-parse HEAD)"
    victim="$(jq -r '.files | keys[] | select(startswith("gate-sdk/"))' "$C/checkwright.lock" | head -n1)"
    [[ -n "$victim" && -f "$C/$victim" ]] || fail "$profile: the manifest records no gate-sdk file for the update --dry-run arm to delete"
    git -C "$C" rm -q -- "$victim" && git -C "$C" commit -q -m "chore: drop $victim" \
        || fail "$profile: could not commit the deletion the update --dry-run arm plans against"
    before="$(git -C "$C" rev-parse 'HEAD^{tree}')"
    out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" update --dry-run 2>&1 )"; rc=$?
    [[ "$rc" -eq 0 ]] || { printf '%s\n' "$out" >&2; fail "$profile: update --dry-run exited $rc"; }
    grep -q '^DRY RUN: nothing was written\.$' <<<"$out" \
        || { printf '%s\n' "$out" >&2; fail "$profile: update --dry-run printed no dry-run verdict"; }
    grep -F "\"$victim\"" <<<"$out" | grep -qF '(pending)' \
        || { printf '%s\n' "$out" >&2; fail "$profile: update --dry-run did not plan to restore the deleted $victim"; }
    [[ "$(git -C "$C" rev-parse 'HEAD^{tree}')" == "$before" && ! -e "$C/$victim" ]] \
        || fail "$profile: update --dry-run wrote to the consumer"
    [[ -z "$(git -C "$C" status --porcelain)" ]] || fail "$profile: update --dry-run left the worktree changed"
    git -C "$C" reset -q --hard "$head" || fail "$profile: could not restore the consumer after the update --dry-run arm"
    say "update --dry-run: planned the deleted $victim pending, tree object and worktree unchanged"

    # spec: installer/SPEC.md §The consumer smoke — the --dry-run rule is asserted behaviorally rather than through a syntactic proxy: the plan must name a non-zero removal count while the tree object and the worktree are both exactly what they were, which is what a flag that parsed and then wrote anyway would fail and a flag that merely existed would pass
    before="$(git -C "$C" rev-parse 'HEAD^{tree}')"
...
# spec: installer/SPEC.md §The consumer smoke — the value assertion is over the whole loop rather than inside it, because which profiles catch a prose defect is derived from the rosters and asserting it per profile would be that derivation copied out. Two claims: some profile catches it at all, and some profile *below the maximum* does — the second is the one that matters, since a defect only the payload-derived profile catches is not value an adopter can choose, it is value they have to take everything for
[[ ${#VALUE_RED[@]} -gt 0 ]] \
...
# spec: installer/SPEC.md §Profiles — assertion 4, deferred out of the profile-invariant arm and made here over the registries the installs above actually wrote. The gate set is the installer's own derivation, so this reads its OUTPUT rather than unioning the per-kit recipes a second time, and the claim strengthens with the move: monotonicity now holds over what an adopter of each profile receives rather than over what a recipe said they would
for pair in "${ORDER[@]}"; do
    IFS=$'\t' read -r a b <<<"$pair"
...
C="$(consumer demo)" || fail "could not build the demo arm's consumer"
out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" init --profile "$PROFILE_MIN" 2>&1 )" \
    || { printf '%s\n' "$out" >&2; blocked "demo arm: init --profile $PROFILE_MIN exited non-zero on the consumer the verb is to run inside"; }
DEMO_SCRATCH="$SCRATCH/demo-scratch"
mkdir -p "$DEMO_SCRATCH" || blocked "demo arm: could not make the scratch base the verb is pointed at"
before="$(git -C "$C" rev-parse 'HEAD^{tree}')"
status="$(git -C "$C" status --porcelain)"
out="$( cd "$C" && DEMO_TMP_DIR="$DEMO_SCRATCH" PATH="$RUN_PATH" "${ENTRY[@]}" demo 2>&1 )"; rc=$?
[[ "$rc" -eq 0 ]] || { printf '%s\n' "$out" >&2; fail "demo arm: checkwright demo exited $rc"; }
grep -q 'DEMO: clean' <<<"$out" || { printf '%s\n' "$out" >&2; fail "demo arm: checkwright demo exited 0 without its DEMO: clean line"; }
# spec: installer/SPEC.md §The consumer smoke — the act 3 red read from the runner's `FAIL:` verdict tail, so the arm couples to no gate name
act3_red="$(awk '/^  ACT 3 /{on=1; next} /^  ACT 4 /{on=0} on' <<<"$out" | grep -c 'FAIL: ')"
[[ "$act3_red" -gt 0 ]] || { printf '%s\n' "$out" >&2; fail "demo arm: checkwright demo printed no FAIL: verdict line between its act 3 and act 4 banners"; }
[[ "$(git -C "$C" rev-parse 'HEAD^{tree}')" == "$before" ]] \
    || fail "demo arm: checkwright demo changed the invoking repository's tree object"
[[ "$(git -C "$C" status --porcelain)" == "$status" ]] \
    || fail "demo arm: checkwright demo left the invoking repository's worktree changed"
[[ -z "$(ls -A "$DEMO_SCRATCH")" ]] \
    || fail "demo arm: checkwright demo left its scratch behind in DEMO_TMP_DIR: $(ls -A "$DEMO_SCRATCH" | tr '\n' ' ')"
say "demo: $(grep -c 'gates passed' <<<"$out") green battery run(s), $act3_red FAIL: verdict line(s) in act 3, the invoking tree and worktree unchanged, the scratch torn down"
out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" demo extra 2>&1 )"; rc=$?
[[ "$rc" -eq 2 ]] || { printf '%s\n' "$out" >&2; fail "demo arm: checkwright demo extra exited $rc, not the usage refusal's 2"; }
grep -q 'usage: checkwright demo' <<<"$out" \
...
# spec: installer/SPEC.md §The consumer smoke — the re-seed leg: a deleted seed whose recorded hash is stale is init's own fresh write, so it is recorded at what was written and never reported as the adopter's change
printf 'plan parity over deleted seeds with stale hashes (%s)\n' "$PROFILE_DERIVED"
C="$(consumer re-seed)" || fail "could not build the re-seed consumer"
out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" init --profile "$PROFILE_DERIVED" 2>&1 )" \
    || { printf '%s\n' "$out" >&2; fail "init failed on the re-seed consumer"; }
STALE="$(printf 'a prior version of this seed\n' | git hash-object --stdin)"
while IFS= read -r p; do
    rm -f "$C/$p" || fail "could not delete $p in the re-seed consumer"
    jq --arg p "$p" --arg h "$STALE" '.files[$p] = $h' "$C/checkwright.lock" > "$C/checkwright.lock.new" \
        && mv "$C/checkwright.lock.new" "$C/checkwright.lock" || fail "could not stale $p's recorded hash"
done <<<"$SEEDED"
git -C "$C" add -A && git -C "$C" commit -q -m "stale seeds" || fail "could not commit the stale seeds"
PLAN="$(dry_plan "$PROFILE_DERIVED" "$C")"
out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" init --profile "$PROFILE_DERIVED" 2>&1 )" \
    || { printf '%s\n' "$out" >&2; fail "init failed re-seeding the re-seed consumer"; }
assert_plan_parity "re-seeded [$(tr '\n' ' ' <<<"$SEEDED")]" "$PLAN" "$C/checkwright.lock"
CHANGED="$(awk '/have changed since init wrote them/ { on = 1; next } /^  help:/ { on = 0 } on' <<<"$out")"
while IFS= read -r p; do
    [[ -f "$C/$p" ]] || fail "re-seed: init did not re-seed the deleted $p"
    [[ "$(jq -r --arg p "$p" '.files[$p]' "$C/checkwright.lock")" == "$(git -C "$C" hash-object -- "$p")" ]] \
        || fail "re-seed: $p is recorded at a hash other than the one init just wrote"
    ! grep -qxF "  $p" <<<"$CHANGED" \
        || { printf '%s\n' "$out" >&2; fail "re-seed: init reported its own re-seed of $p as a changed file"; }
done <<<"$SEEDED"
say "re-seed: $(wc -l <<<"$SEEDED" | tr -d ' ') deleted seed(s) re-seeded, recorded at what was written, none reported changed"
...
# spec: installer/SPEC.md §The consumer smoke — the refusal is asserted on the CONSUMER and not only on the exit code: a refusal that wrote first and refused after would exit non-zero too, and only an untouched tree tells the two apart
...
# spec: installer/SPEC.md §The consumer smoke — the download transport, asserted rather than documented: verify the digest, extract with tar rather than npm, and drive the same post-conditions with node/npm masked, so a latent Node dependency reds here instead of passing on a host that happens to carry Node
...
# spec: installer/SPEC.md §The consumer smoke — the mask is the whole value of this arm: a shim that reds and names itself turns a latent Node reach into a loud failure rather than a silent pass, and shims are what keeps the mask portable — dropping every PATH entry that carries node would take /usr/bin with it on the hosts where node lives there
MASK="$SCRATCH/mask"
mkdir -p "$MASK"
...
# spec: installer/SPEC.md §The consumer smoke — masking is per-arm, which is what lets this arm exist without weakening the preflight the build step depends on; the mask is proved rather than assumed for the same reason the Node-free one is
for masked in cargo rustc; do
    resolved="$( PATH="$RUN_PATH" bash -c "command -v $masked" 2>/dev/null )"
...
# spec: installer/SPEC.md §doctor — doctor is asserted before init as well as inside it, because init reads only its exit status: an adopter meets this verdict first, and it is the precondition every later refusal is downstream of
out="$( cd "$C" && PATH="$RUN_PATH" "${ENTRY[@]}" doctor 2>&1 )"; rc=$?
[[ "$rc" -eq 0 ]] \
...
path_without() {   # $1 = the program stem to mask, $2 = the farm dir to build -> the masked PATH on stdout
    ...
    for d in "${dirs[@]}"; do
        ...
        here=0
        for f in "$d"/*; do
            stem_is "$stem" "$f" || continue
            here=1; break
        done
        if [[ "$here" -eq 0 ]]; then keep+=("$d"); continue; fi
        for f in "$d"/*; do
            b="${f##*/}"
            stem_is "$stem" "$f" && continue
            [[ -x "$f" && ! -d "$f" ]] || continue
...
# spec: installer/SPEC.md §The consumer smoke — the JSON-reading verbs are asserted to SUCCEED here, which is the whole inversion: a verb that still shelled out to jq would fail on this PATH, and one that reads the manifest in-process cannot tell the difference. The label is carried so a red names which verb reached for a program that is not there
...
# spec: installer/SPEC.md §doctor — jq is a contributor member, so doctor names it nowhere: not unprobed with no install, and not inside any install
assert_doctor_silent() {   # $1 = a label for the message, $2 = consumer dir
    local out rc
    out="$( cd "$2" && PATH="$JQ_PATH" "${ENTRY[@]}" doctor 2>&1 )"; rc=$?
    [[ "$rc" -eq 0 ]] \
        || { printf '%s\n' "$out" >&2; fail "doctor ($1) exited $rc with jq absent — no selection owes jq, so its absence cannot set the verdict"; }
    grep -q 'jq' <<<"$out" \
        && { printf '%s\n' "$out" >&2; fail "doctor ($1) named jq — a contributor member is skipped outright, so an adopter is told about a program nothing they install runs"; }
    say "doctor ($1): clean, and names jq nowhere"
}
assert_doctor_silent "no install" "$C"

out="$( cd "$C" && PATH="$JQ_PATH" "${ENTRY[@]}" init --profile "$PROFILE_MIN" 2>&1 )" \
    || { printf '%s\n' "$out" >&2; fail "init --profile $PROFILE_MIN refused on a jq-less machine — no profile's floor carries jq"; }
say "init --profile $PROFILE_MIN: installs with no jq on PATH, exit 0"
assert_doctor_silent "inside $PROFILE_MIN" "$C"

# spec: installer/SPEC.md §The consumer smoke — the kit whose hook once spawned jq is named here because the arm drives that hook, and the profile carrying it is found through the harness's own profile reader; a fresh consumer keeps the install isolated from every manifest question above
GUARD_KIT=guard-kit
GUARD_PROFILE=""
for p in "${PROFILES[@]}"; do
    mapfile -t guard_p_kits < <(profile_kits "$p")
    for k in "${guard_p_kits[@]}"; do
        [[ "$k" == "$GUARD_KIT" ]] && { GUARD_PROFILE="$p"; break 2; }
    done
done
[[ -n "$GUARD_PROFILE" ]] || fail "no profile carries $GUARD_KIT, so the arm cannot assert that its hook runs on a jq-less machine"
C2="$(consumer jq-less-guard)" || fail "could not build a second scratch consumer for the jq-less arm"
out="$( cd "$C2" && PATH="$JQ_PATH" "${ENTRY[@]}" init --profile "$GUARD_PROFILE" 2>&1 )" \
    || { printf '%s\n' "$out" >&2; fail "init --profile $GUARD_PROFILE refused on a jq-less machine — $GUARD_KIT's hook reads JSON through the gate binary, so the profile owes no jq"; }
say "init --profile $GUARD_PROFILE: installs with no jq on PATH, exit 0"
assert_doctor_silent "inside $GUARD_PROFILE" "$C2"
...
# spec: installer/SPEC.md §init — the adopter's edit is committed, because init refuses a dirty worktree: the case under test is a file changed since init wrote it, not an uncommitted one
...
# spec: installer/SPEC.md §The consumer smoke — the relinquish is performed on the extracted package's own payload rather than through a pack flag: pack-installer assembles every version from one worktree, so without this the two hops carry byte-identical payloads and no path ever leaves a kit's shipped set. Mutating the test's own extracted copy keeps the publishing path with no way to ship a payload with a hole in it
...
# spec: installer/SPEC.md §The manifest — a path leaves the roster when the file leaves the tree and at no other moment, so the hop whose payload stopped shipping it must still carry it at the hash init wrote there: dropping it here is what makes the re-adding hop below read the path as never installed
...
# spec: installer/SPEC.md §The consumer smoke — the second hop is the one the first cannot stand in for: one upgrade shows the protection starting, and only the next shows whether it persists or inverts, so the same consumer is carried across a third version with no fresh adopter edit
...
# spec: installer/SPEC.md §The manifest — the re-adding hop is where the ownership rule pays: this payload ships $RELINQUISHED again, so it must meet the carried claim and refuse, which is the whole defect reproduced end to end rather than argued about
[[ "$(git hash-object -- "$C/$RELINQUISHED")" == "$R_WANT" ]] \
...
# spec: installer/SPEC.md §What init seeds — the two surfaces init rewrites on every run: a templates/*-config.knobs destination and gate-sdk's msg-patterns.list. The arm runs at the maximum profile because that is the only profile whose kit set is fixed by the payload rather than by a roster judgment, so it is where both surfaces are present by construction — a smaller profile would tie the arm to a membership row that is a judgment and may be revised
...
# spec: installer/SPEC.md §The consumer smoke — the protection branch chains onto this arm rather than the reversal arm, because an adopter edit is exactly the case tree-object equality cannot host: this consumer already carries two edited, committed vendored files, which is the case that reaches uninstall's keep branch and the residual manifest behind it
declare -A SEAM_KEPT=()
...
[[ "${#_seam_sorted[@]}" -eq "${#_seam_stored[@]}" ]] \
    || fail "the residual manifest re-renders to ${#_seam_sorted[@]} line(s) where the stored file carries ${#_seam_stored[@]}"
[[ "${_seam_sorted[*]}" == "${_seam_stored[*]}" ]] \
...
# spec: installer/SPEC.md §The manifest — the narrowing arm, and it is the only arm that moves a consumer *down* the lattice: every other re-run holds the profile fixed, so none reaches the state where files[] outlives kits. That state is not exotic — it is the ordinary consequence of the carry-forward rule, which keeps every once-vendored path on the roster while the recorded kit set shrinks
...
# spec: installer/SPEC.md §The manifest — the residual shape is the same defect with the kits key absent rather than shrunk, so it is asserted on a kits-stripped copy of this manifest rather than bought a second consumer: uninstall's residual carries schema and files only, and a resolver leaning on the recorded kit set excludes nothing at all there
jq 'del(.kits)' "$NARROW_LOCK" > "$NC2/residual-shape.json" \
    || fail "could not derive the residual manifest shape"

# spec: installer/SPEC.md §The manifest — only the seam paths this install actually recorded are asserted on, because the config seam is written on the artifact placement path alone: a payload carrying no prebuilt binary records none, and demanding it here would fail the arm on the payload rather than on the resolver. The arm proves its own premise instead — at least one checked path, and at least one of them genuinely shadowed by a vendored fixture tree, or a green result would mean only that the payload changed shape
narrow_checked=0
narrow_shadowed=0
...
[[ "$narrow_shadowed" -gt 0 ]] || {
    ...
    # spec: installer/SPEC.md §The consumer smoke — the sample is bounded inside jq rather than by closing a pipe under it, so the diagnostic does not print a write error of its own beside the failure it is explaining
...
# spec: installer/SPEC.md §The consumer smoke — the hand-off arm adopts a producer's bytes and sidecar unchanged, so the marker states the binary's provenance rather than asserting this run built one
