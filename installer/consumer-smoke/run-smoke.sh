...
# spec: installer/SPEC.md §The consumer smoke — the front page's proof block is a copy of act 3, held line by line against the output it was copied from
proof="$(awk '/^<!-- demo-proof:begin -->/{on=1; next} /^<!-- demo-proof:end -->/{on=0} on' "$REPO/docs/index.md" | grep -v -e '^```' -e '^[[:space:]]*$')"
[[ -n "$proof" ]] || fail "demo arm: docs/index.md carries no demo-proof block to hold against the demo's output"
while IFS= read -r line; do
    grep -qF -- "$line" <<<"$out" \
        || { printf '%s\n' "$out" >&2; fail "demo arm: a line of docs/index.md's demo-proof block is not in the demo's output — recapture it from this run: $line"; }
done <<<"$proof"
[[ "$(git -C "$C" rev-parse 'HEAD^{tree}')" == "$before" ]] \
...
say "demo: $(grep -c 'gates passed' <<<"$out") green battery run(s), $act3_red FAIL: verdict line(s) in act 3, $(wc -l <<<"$proof" | tr -d ' ') front-page proof line(s) found in the output, the invoking tree and worktree unchanged, the scratch torn down"
