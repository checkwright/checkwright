#!/usr/bin/env pwsh
# spec: installer/SPEC.md §The install boundary — the second hand-kept bootstrap, authored against that section's five steps in the one language a native Windows host runs without a POSIX shell; installer/bin/checkwright.sh is its twin, not its original and not a transliteration of it
#
...
# spec: installer/SPEC.md §The gate binary — the detector's whole input, factored out so a refusal can name what the host was detected AS rather than only that it mapped to nothing; the twin of installer/bin/checkwright.sh's host_shape()
