...
The mechanism behind the activation path: what the bootstraps hand to, what each
verb does and refuses, what an install records, how the package is assembled, and
what asserts all of it end to end. `installer/README.md` is the usage tier in
front of this file — what an adopter runs; this file is why it is built that way.

This file is not named on `installer/package.json`'s `files` roster, so it ships
in neither the npm tarball nor the Release tarball. §Layout rules that the roster
and not the directory listing decides what is reachable at an installed
`PKG_ROOT`, which is the same rule the payload applies to a kit's `SPEC.md`,
reaching the package one layer out.

## The dependency boundary

What this package is not: a dependency channel. Nothing resolves at your build
...
**The generation step spawns only the binary `init` placed.** The hooks and the
coupling graph are generated by the binary's own `--emit git-hooks --write` and
`--emit graph` arms (gate-sdk/SPEC.md §gen-pre-commit), and `check-graph` is
`install: zero-config`, so a fresh consumer's day-one battery holds the generated
hook against its emission and the hook must therefore exist at install. The step
is consequently not droppable. `init` runs the artifact it has just placed at the
consumer's root with the gate-sdk root locator `GATE_SDK_ROOT` in the child's
environment, naming the root-vendored `gate-sdk` directory exactly as the front-end
would have exported it (gate-sdk/SPEC.md §Layout and configuration), and the front-end's
only other duties (changing to the root, locating the binary) are ones `init`
already holds. So `init` spawns only `git` and that binary, no `bash`, and the
*bootstrap* — which is what the interpreter policy's standing "assume no POSIX
shell" obligation binds — spawns nothing.
