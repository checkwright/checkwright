...
**A `behind-invoke` step may spawn `bash`, and one does.** The hooks are
generated by the binary's own `--emit git-hooks` arm (gate-sdk/SPEC.md
§gen-pre-commit), but the step reaches it through the vendored front-end stub
`gate-sdk/bin/run-gates.sh`, which is bash, and `check-graph` is
...
is consequently not droppable. It is **not** stuck:
the port criteria clear a `bash` spawn explicitly because `bash` is on
...
happens to sit on (context-kit/SPEC.md §bin/env-probe). So the step sits behind the invoke
