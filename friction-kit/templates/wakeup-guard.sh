#!/usr/bin/env bash
# spec: friction-kit/SPEC.md §wakeup-guard — PreToolUse(ScheduleWakeup|CronCreate)
...
source "$FRICTION_KIT_LIB" 2>/dev/null || true
: "${FRICTION_KIT_WAKEUP_LOG:=.workflow/wakeup-attempts.log}"
...
input="$(cat 2>/dev/null || true)"
{
    ...
    printf '%s\n' "$input" | jq -c '{session_id, tool_name, tool_input}' 2>/dev/null \
        || printf '%s\n' "$input"
} >>"$FRICTION_KIT_WAKEUP_LOG" 2>/dev/null || true
