#!/usr/bin/env bash
# Behavioral test of the off-root rule in check-docs-link-convention.
...
check_case() {
    local out rc
    out="$(cd "$2" && gate_run check-docs-link-convention "$CHECKS" docs 2>&1)"; rc=$?
    if [[ "$rc" -ne "$3" ]]; then
...
out_f="$(cd "$tmp/f" && gate_run check-docs-link-convention "$CHECKS" docs 2>&1)"
