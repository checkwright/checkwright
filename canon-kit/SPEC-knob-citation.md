...
- New gate `canon-kit/checks/check-knob-citation.sh` — the enforcement
  ...
  prefix matches a kit from `gate_kit_roots` (kit dir name uppercased,
  hyphens to underscores — the `delegation-kit` → `DELEGATION_KIT_`
  shape, with the sdk's own dir mapping the same way), so the gate ships
  no term list and the provenance seam is untouched.
