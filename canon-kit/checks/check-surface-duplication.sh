...
GLOSS="$ROOT/$CANON_KIT_GLOSSARY_FILE"
[[ -f "$GLOSS" ]] || { echo "check-surface-duplication: no $CANON_KIT_GLOSSARY_FILE at $GLOSS (register the gate only where the glossary topology exists)" >&2; exit 2; }

declare -a SURFACES=()
COMPONENTS=""
for s in "${CANON_KIT_DUP_SURFACES[@]}"; do
    [[ -f "$ROOT/$s" ]] || continue
    if [[ "$(basename "$s")" == "$CANON_KIT_SPEC_NAME" ]]; then
...
for entry in "${SURFACES[@]}"; do
    ...
    while IFS=$'\t' read -r head lineno valve; do
        ...
        [[ "$valve" == "$head" ]] && continue
        errors+=("$rel:$lineno: bold-lead-in definition of glossary term '$head' — this surface may name it and explain *why*/*how-it-joins*, not carry the canonical definition (that lives in $CANON_KIT_GLOSSARY_FILE). Reword to narration + a pointer, or tag the line '<!-- $vk: $head -->' if this surface legitimately introduces it")
    done <<< "$out"
done
...
if (( ${#errors[@]} )); then
    ...
    printf '  %s\n' "${errors[@]}"
    echo "  help: $CANON_KIT_GLOSSARY_FILE owns the definition; another surface owns the *why* and the local mechanism — reword a restated definition to narration + a pointer, or tag the line '<!-- vision-introduces: <term> -->' / '<!-- spec-introduces: <term> -->'"
    exit 1
fi
