...
# shellcheck disable=SC2034  # nameref-consumed by build_bless
txt_word=(see)
txt_colon+=("${CANON_KIT_COMMENT_MACHINE[@]}" "${CANON_KIT_COMMENT_REASON[@]}")
...
POS_RE="$(join_alt "${CANON_KIT_COMMENT_POSITIONAL[@]}")"
EXEMPT_RE="$(join_alt comment-tier-exempt:)"

# spec: canon-kit/SPEC.md §check-comment-tier — the classifier walks full-line
...
# spec: canon-kit/SPEC.md §check-comment-tier — the count-shape override rides the shared adapter, so the noun vocabulary and wedge window enter once
...
# spec: canon-kit/SPEC.md §check-comment-tier — the governed surface via
