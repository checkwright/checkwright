...
#   (default $CANON_KIT_QUEUE_FILE): live task resolves, done task is stale,
...
# spec: canon-kit/SPEC.md §check-todo-task-liveness — the queue-resolution pass is
...
for f in "${SURFACE[@]}"; do
    rel="${f#"$ROOT"/}"; rel="${rel#./}"
    scanned=$((scanned + 1))
    # spec: canon-kit/SPEC.md §check-todo-task-liveness — the marker needs a
