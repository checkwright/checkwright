...
# spec: canon-kit/SPEC.md §check-manifest-count — prose walk only: the shared driver gates fences and per-site markers, these hooks judge the line (sk_count_hit) and the wrapped paragraph (sk_para_wrapped)
...
if [[ -n "$out" ]]; then
    ...
    echo ""
    echo "$out"
    echo "  help: reword to cite the owning collection (e.g. 'the gates in gates.list') rather than pin a total; a genuinely fixed named set joins CANON_KIT_COUNT_ALLOWED_PHRASES; a threshold/rate/partition/proportion is already exempt; else add a 'manifest-count-exempt: <reason>' comment on the line or the one above"
    exit 1
fi
