...
# spec: canon-kit/SPEC.md §check-spec-embedded-source — a canonical spec's fenced block must not verbatim-copy a tracked source file
set -uo pipefail
...
find_globs=()
for entry in "${CANON_KIT_EMBED_LANGS[@]}"; do
...
for l in "${CANON_KIT_EMBED_ILLUSTRATIVE[@]}"; do unset "LANG2KIND[$l]"; done
...
flagged="$(awk -v threshold="$CANON_KIT_EMBED_THRESHOLD" -v minlines="$CANON_KIT_EMBED_MINLINES" \
               -v langmap="$langmap" -v extmap="$extmap" -v basemap="$basemap" \
               -v amendset="$amendset" -v wirekind="$CANON_KIT_EMBED_WIRE_KIND" '
    ...
    function emit_block(   l, parts, i, f, best, bestf, frac, nd) {
        ...
        # spec: canon-kit/SPEC.md §check-spec-embedded-source — amendment wire-delta exemption
