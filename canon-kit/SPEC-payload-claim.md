...
yet. This iteration ends that: `native-gate-cohort-descriptors` lands the first
cohort's descriptors, and at that commit the §init sentence becomes a false
statement rather than an incomplete one.

**Restored 2026-08-07, not rewritten.** This design was authored 2026-08-06 and
hardened at align the same day; the 2026-08-06 build demoted the entry and
deleted the file when the cohort held its descriptors. It is recovered from
`git show 71c584e:canon-kit/SPEC-payload-claim.md` — the **align-hardened**
revision, not the authoring one, whose corpus align superseded. Nothing of it
was on disk at restoration: no gate, no vocabulary script, no
`spec_claim_vocabulary`, no knobs, no declaration marker. This is a clean
re-promotion.

## The tense ruling, re-read rather than adopted

The entry requires this and is right to: the ruling was written for an iteration
that lands a descriptor, and inheriting it without re-checking that condition is
how the 2026-08-06 demotion became necessary in the first place.

**Re-read, and it stands — on a firmer footing than when written.** The ruling is
that *the corrected claim is written in the present, describing the tree as it
will be at the end of this iteration, and no surface hedges toward a future.* Its
condition is that the port goes live in the same iteration as the correction.
That condition holds: `native-gate-cohort-descriptors` is unit 3 of this
iteration's four, and this entry is unit 4.

**What has changed is the window, and it changed in this ruling's favour.** The
original recorded a sequencing hazard — *"if the disclosure correction lands in a
build batch before the cohort's descriptors, the prose is briefly ahead of the
tree"* — and closed it with a Definition-of-Done clause binding both to one
iteration rather than to one commit. That clause is now belt over braces: the
operator fixed a forced build order (TRAJECTORY.md §The closed rulings) in which
this unit is **last**, after the descriptors. The window is closed structurally,
by ordering, and the DoD clause is retained as the backstop for a batch that
reorders.

**What has not changed is the reason the ruling is not a hedge.** A surface that
says *the ruled direction is* or *today every gate is a shell script* is
describing a transition, and a transition described in prose is a thing a reader
must date. The rule is dateless: it says what a gate discloses **by rule**, not
how many gates are compiled today.

## The finding the entry does not carry: the count itself is the defect

Re-verified against the tree at authoring, the corpus was measured at **four**
claims across two documents. Re-verified again at align, against delta 3's own
`all-source` boundary rather than against the authoring pass's list, it is larger
than that and spans a **third** document:
...
**The prediction came true, and that is now evidence rather than a worry.** A
2026-08-07 scope pass re-enumerated this corpus a **third** time, proposed
four-to-five against the superseded base of four, and routed in a sentence from
a different claim class entirely. It was retracted the same day. The entry now
carries **no count at all**, deliberately, and neither should any surface but
this one — the enumeration lives here, behind the oracle. The list below was
witnessed present and verbatim unchanged at restoration
(`.workflow/survey-record.md`, 2026-08-07 spec), so build cites that witness
rather than re-measuring and becoming the fourth pass.
...
**One axis is now named and waiting, which sharpens that trigger from
hypothetical to scheduled.** `native-gate-cohort-descriptors` files a
**release-asset** claim class — "every tagged release publishes a prebuilt binary
for each platform" — as accepted-ungated with a filed trigger, because it has
exactly one instance. That is the second axis. Neither is built as a registry
here; what changes is that the reversal condition above now has a named candidate
rather than an imagined one, and the two entries point at each other so whichever
fires second finds the design already reasoned.
...
- **`docs/install.md` §What a gate discloses** — retensed. "Today every gate is a
  ...
  its narrow reason paragraph unchanged. **That "most gates are shell today"
  line was wrong when it was drafted and is true only once unit 3 lands** — which
  is exactly why this unit is ordered after it, and why the line is a statement
  about the corpus that must not migrate into the contract sentence beside it.
- **`SECURITY.md` §Threat boundary** — "Vendoring is a copy you read before you
...
- **Not updated here:** `docs/install.md` §Requirements' cargo bullet belongs to
  `native-gate-cohort-descriptors` **entire**. Its release-asset sentence is a
  different claim class — what a release has *published*, not what a consumer
  *receives* — and a batch that corrects it here would split one bullet across
  two units and leave it incoherent between them.
...
- [ ] **No governed-doc line matches `all-source` outside its declared form —
      ...
      from four to seven in one iteration against the same tree, and a third
      pass at scope then proposed a fifth off the superseded base and was
      retracted. That is the failure mode a checked-box tally cannot see itself
      fail, now attested three times. The `bad/` fixture holds one
      corrected-to-original sentence verbatim, so a reintroduction reds rather
      than a count going stale.
- [ ] **The corpus is cited, not re-measured.** `.workflow/survey-record.md`
      carries the witness (2026-08-07 spec) that all seven sites plus the H1
      preamble are present and verbatim unchanged. Run its witness; do not
      become the fourth enumeration.
...
- [ ] **Ships in the same iteration as the cohort, and after it** — the
      correction and `native-gate-cohort-descriptors`' descriptors land in one
      iteration, in that order, so no release carries a tree the corrected prose
      is false about and no batch publishes prose ahead of the tree.
