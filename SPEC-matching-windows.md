...
Three kit gates relate two things they matched across a distance that is a bare number in the module, with no knob. `check-knob-citation` binds a knob token to a following "default" marker within 100 code points (`MARKER_REACH`, `native/src/gates/knob_citation.rs`). `check-knob-default-coupling` looks for a knob's default statement within 400 code points after the knob's name in the owning SPEC, an inline literal no SPEC sentence states (`native/src/gates/knob_default_coupling.rs`). `check-assertion-strength` reads a guard's failure text within 8 lines after the call (`WINDOW`, `native/src/gates/assertion_strength.rs`). And the default-statement grammar (`spec::DefaultGrammar::default_bound_at`, `native/src/spec.rs`) reads the word "default" as a value marker only when a value literal lies wholly within the 24 bytes after it, the "short window" canon-kit/SPEC.md §check-knob-citation names without a value. That grammar has one caller of this span, `check-knob-citation`. `check-knob-default-coupling` shares the grammar's literal reader but hands it the tail of its own window, so the 24 bytes never reach it. Each of the four passes the discriminator in doctrine-kit's Policy-as-choice rule: another value keeps the gate's stated contract true, so an adopter whose prose or scripts sit at a different distance meets a red, or a miss, that they can only bypass.
...
- `CANON_KIT_KNOB_CITATION_LITERAL_SPAN`: a byte count, default `24`. It has no `off` of its own, because it bounds the same leg `CANON_KIT_KNOB_CITATION_REACH`'s `off` already stops, and a second switch for one leg would be two spellings of one choice.

This repo binds each knob to its shipped value in its own knob files.

**Why the literal span takes no structural value.** The span exists to tell a stated default ("default `7`", "defaults to `7`") from the adverbial reading ("empty by default so the gate falls through to the live `git` read", a sentence gate-sdk/SPEC.md §check-identity carries). The only units the grammar defines wider than the phrase are the clause and the sentence, and both admit the adverbial reading with a literal later on, which is the false red the span holds off. A closed list of connector words would be structure in name only: a phrase list is itself a calibration. The span stays in bytes, as it shipped, because a code-point count moves the verdict on a line where multi-byte punctuation sits between the word and the literal.
...
**Why this repo binds the shipped values.** `100` is the reach this tree was measured at when its markdown was unwrapped. `24` is the span the citation gate is green at today. `400` and `8` move no verdict here: `check-assertion-strength` reaches zero calls on this tree (its clean line reads `0 call(s) to a script with a declared exit contract`), and the coupling gate is green at `400`. `sentence` has not been measured on this tree, and switching to it would be a verdict change that this unit does not take.
...
### (2) §check-knob-citation states the reach and the literal span as knobs {mechanical}
...
> The marker is one of two shapes: `=` appended directly to the token (`<KIT>_<KNOB>=<value>`), or the word "default" following the token within the reach `CANON_KIT_KNOB_CITATION_REACH` sets and bound to a value literal (a backticked value, a quoted string, or a number) by the literal span below. A marker before the token binds nothing, since a stated default reads knob first, then value. The reach defaults to `100` code points. It may instead be any positive integer, or `sentence`, which binds a marker only when no sentence terminator (`.`, `?`, `!` or `;` before whitespace) sits between it and the token, or `off`, which stops reading the default shape so that only the `=` shape fires, and the clean line says so. Any other value exits 2. The reach is the consumer's to set, because the right distance depends on the tree. On a wrapped tree the line already bounds the leg. On an unwrapped tree a paragraph is one line, and an unbounded leg would bind a knob to another knob's default further along it. The default is the reach the leg shipped with, so an upgrade moves no verdict. `sentence` is the structural bound, with the honest limit that an abbreviation's period ends a sentence early.

In the paragraph beginning "Two calibrations hold the false-positive rate", the sentence beginning "And the "default" leg binds:" becomes:

> And the "default" leg binds only a literal lying wholly within the `CANON_KIT_KNOB_CITATION_LITERAL_SPAN` bytes after the word, default `24`, that is not itself a backticked knob name. So a stated default like ``default `docs/CNAME` `` fires, while a knob cited bare after the verb ("these paths default through `<KIT>_<KNOB>`") does not, and neither does the adverbial "by default" with a literal later in its sentence. The span takes any positive integer, and any other value exits 2. It has no `off`, since the reach's `off` already stops the leg, and no sentence or clause value, since either admits the adverbial reading the span exists to refuse.
...
### (7) This repo binds its four choices {mechanical}

**Not yet applied.** `scripts/canon-config.knobs` gains `CANON_KIT_KNOB_CITATION_REACH = 100`, `CANON_KIT_KNOB_CITATION_LITERAL_SPAN = 24` and `CANON_KIT_DEFAULT_COUPLING_WINDOW = 400`, and `scripts/gate-sdk-config.knobs` gains `GATE_SDK_ASSERTION_STRENGTH_WINDOW = 8`. Each line is led by a `# spec:` pointer to its gate's section. The preamble gives the grounds, which are this repo's and stay out of the kit SPEC.
...
**Not yet applied.** `.workflow/release-declarations.md` §Behavior changes gains one bullet per new knob. Each names the knob and its gate, says the default is the value the gate shipped with so no verdict moves, and names the alternatives (`sentence` and `off` for the citation reach, `off` for the coupling window, none for the assertion-strength window or the literal span). No bullet states a knob beside its default literal: the composed note publishes under `docs/posts/`, a manifest glob this repo configures, where `check-knob-citation` reds a knob stated with its value outside its owning SPEC.

### (9) The knob `CANON_KIT_KNOB_CITATION_LITERAL_SPAN` {design-bearing}

**Not yet applied.** canon-kit's table gains the scalar `CANON_KIT_KNOB_CITATION_LITERAL_SPAN`, default `24`. Its validator refuses any value other than a positive integer, at exit 2.

`spec::DefaultGrammar::default_bound_at` takes the span as a parameter, replacing the inline `24`. The span stays in bytes, and a literal must still close inside it, so the default reproduces today's markers exactly. `check-knob-citation` reads the knob once per run and passes it. `check-knob-default-coupling` never calls `default_bound_at`, so it neither reads nor declares the knob. The `check-knob-citation` registry entry declares it beside the reach from delta 1.

Fixtures: the pair keeps its config. `canon-kit/gate-tests/check-knob-citation.test.sh` gains cases over a line where a knob is followed by "defaults, as the kit table has it, to" and then a backticked value about 30 bytes after the word. The case is clean at the default, red at `40`, and exits 2 on an unknown value. A unit test in `native/src/spec.rs` holds `default_bound_at` at spans of 24 and 40 over that same line.

### (10) §The shared spec adapters says the span is the caller's {mechanical}

**Not yet applied.** In canon-kit/SPEC.md §The shared spec adapters, the default-statement grammar bullet's sentence "It recognizes the value literal opening a window (a backticked non-knob string, a quoted string, or a number) and returns the literal the word "default" binds; a caller-supplied knob-name predicate keeps a bare knob name from reading as a value." becomes:

> It recognizes the value literal opening a window (a backticked non-knob string, a quoted string, or a number) and returns the literal the word "default" binds. The caller supplies the window's width and a knob-name predicate that keeps a bare knob name from reading as a value. `check-knob-citation` passes its literal span (§check-knob-citation), and `check-knob-default-coupling` passes the tail of its own SPEC window.

## Producers and consumers

- **The four values** (deltas 1, 3, 5 and 9). Producer: each kit's table, overridden by a consumer's knob file; this repo sets all four (delta 7). Consumers: `check-knob-citation`, at the per-line marker binding (the reach) and inside `default_bound_at` (the span); `check-knob-default-coupling`, at each knob's SPEC read; `check-assertion-strength`, at each declaring call's window. Each is read once per run. The canon validator reads its three at the kit's first resolution, and `check-assertion-strength` validates its own.
- **Point 2, roster readers of the minted names.** The registry declarations in `native/src/gates/mod.rs` (a knob a member reads undeclared is a finding under the crate's declaration tests). `check-knob-default-coupling` couples each new scalar row's default to its SPEC statement, which is why deltas 2, 4 and 6 state each default as a literal. Delta 2's text was read against that gate's rule: the first "default" inside each knob's window binds that knob's own literal. `check-kit-ref-liveness` resolves each name to its table row. `--emit knob-roster` and `check-docs-cmd`'s static knob names derive from the tables and need no edit. The knob-file couple is derived (gate-sdk/SPEC.md §The `# graph:` manifest), and the pre-commit hook already carries it for all three gates, and already fires `check-knob-citation` on `native/src/spec.rs`.
...
- `native/src/gates/mod.rs`, four registry declarations (deltas 1, 3, 5 and 9).
- `canon-kit/gate-tests/check-knob-citation.test.sh`, the reach cases (delta 1) and the span cases (delta 9).
...
- `canon-kit/SPEC.md` §check-knob-citation (delta 2), §check-knob-default-coupling (delta 4) and §The shared spec adapters (delta 10).
...
- [ ] **Causal completeness.** Every point of canon-kit/SPEC.md §The causal-completeness check holds for the four knobs.
