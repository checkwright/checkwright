...
# spec: spec-kit/SPEC.md §check-prose-enum — within one manifest-prose paragraph, naming two or more members of a declared governed set must name every member, unless an exempt site holds
set -uo pipefail
...
if [[ -z "$SPEC_KIT_ENUM_SETS_CMD" ]]; then
    echo "PROSE-ENUM: clean (SPEC_KIT_ENUM_SETS_CMD unset — no declared sets to check)"
    exit 0
fi

sets="$(spec_enum_sets)"; st=$?
fail_closed "$st" check-prose-enum "SPEC_KIT_ENUM_SETS_CMD"
[[ -n "$sets" ]] || { echo "PROSE-ENUM: clean (SPEC_KIT_ENUM_SETS_CMD declared no members)"; exit 0; }
...
# spec: spec-kit/SPEC.md §check-prose-enum — the enum matcher and hooks: word-bounded member presence (bracketed or bare, neither alnum nor hyphen abutting), the delimited-adjacency test that separates a hand list from scattered mentions, the subset/partitive exempt escapes, and the per-paragraph verdict the shared driver flushes
