...
# spec: spec-kit/SPEC.md §check-amendment-queue — the Task↔amendment bidirectional rule and spec-readiness
#
# usage: check-amendment-queue.sh [queue-file [scan-root]]
#   Defaults to the configured queue file (SPEC_KIT_QUEUE_FILE) and '.' for the
...
(( ${#an[@]} ))      && errors+="[needs-spec] tag in an active-queue entry (move it to $SPEC_KIT_DEFERRED_SECTION):"$'\n'"$(printf '  %s\n' "${an[@]}")"$'\n'
(( ${#pn[@]} ))      && errors+="[needs-spec] tag in active-queue prose ($SPEC_KIT_DEFERRED_SECTION-only tag; say \"needs design\" in prose):"$'\n'"$(printf '  %s\n' "${pn[@]}")"$'\n'
(( ${#dopen[@]} ))   && errors+="$SPEC_KIT_DEFERRED_SECTION entries without [needs-spec] (all deferred work is design-pending):"$'\n'"$(printf '  %s\n' "${dopen[@]}")"$'\n'
(( ${#dready[@]} ))  && errors+="$SPEC_KIT_DEFERRED_SECTION entries already carrying [spec:] (promote to a feature section):"$'\n'"$(printf '  %s\n' "${dready[@]}")"$'\n'
...
if [[ -n "$errors" ]]; then
    ...
    echo ""
    printf '%s' "$errors"
    echo "  help: pair every amendment with a [spec: …] queue entry and vice versa; tag every $SPEC_KIT_DEFERRED_SECTION entry [needs-spec]; give every feature entry a [spec:] ref"
    exit 1
fi
