...
# spec: spec-kit/SPEC.md §check-spec-fence-balance — every governed markdown file has an even fence-delimiter count, so the fence-skipping parsers never desync and fail open
#
...
if [[ $# -gt 0 ]]; then
    ...
    [[ -f "$SPEC_KIT_QUEUE_FILE" ]] && files+=("$SPEC_KIT_QUEUE_FILE")
fi
