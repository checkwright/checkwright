...
# spec: spec-kit/SPEC.md §check-docs-cmd — every fenced invoked repo-relative .sh path and every backticked/fenced kit-prefixed env knob in the governed doc set resolves against the tree
#
# usage: check-docs-cmd.sh [file...]
#   Defaults to the manifest set (lib/spec.sh) minus SPEC_KIT_MDREF_EXCLUDE — the
...
if [[ ${#bad[@]} -gt 0 ]]; then
    ...
    echo "        joins SPEC_KIT_MDREF_EXCLUDE. Only invoked .sh paths and kit-prefixed knobs count."
    exit 1
fi
