...
out="$(awk -v banned="$banned" -v density="$SPEC_KIT_DERIVABLE_DENSITY" \
           -v pointer="$SPEC_KIT_DERIVABLE_POINTER_REGEX" '
    function flush(   dens) {
...
if [[ -n "$out" ]]; then
    ...
    printf '%s\n' "$out" | sed 's/^/  /'
    echo "  help: a banned-heading section (${SPEC_KIT_BANNED_HEADINGS[*]}) that is mostly a code dump drifts — shed the body to a one-line index pointer (cite the code), keeping the prose that owns semantics"
    exit 1
fi
echo "SPEC-DERIVABLE-SECTION: clean (${#specs[@]} $SPEC_KIT_SPEC_NAME, no banned-heading section exceeds the ${SPEC_KIT_DERIVABLE_DENSITY}% fenced budget)"
exit 0
