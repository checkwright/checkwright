...
# spec: spec-kit/SPEC.md §check-deprecation-task — every deprecation marker on a governed source binds task: <slug> to a live queue task
#
# usage: check-deprecation-task.sh [scan-root [queue-file]]
#   Resolves each SPEC_KIT_DEPRECATION_MARKERS marker's task: <slug> binding
#   against the queue (default $SPEC_KIT_QUEUE_FILE); an empty roster is a skip.
set -uo pipefail
...
if [[ ${#SPEC_KIT_DEPRECATION_MARKERS[@]} -eq 0 ]]; then
    echo "DEPRECATION-TASK: clean (no SPEC_KIT_DEPRECATION_MARKERS configured; the deprecation-marker vocabulary is consumer config — nothing to resolve)"
    exit 0
fi
...
# spec: spec-kit/SPEC.md §check-deprecation-task — the queue-resolution pass is
