...
- `aarch64-unknown-linux-gnu` (joined) — Linux on arm64. The gates workflow builds this target on an arm64 Linux runner, and `install-smoke-sh-linux-arm64` installs that build — the producer's own upload, nothing rebuilt — reaching its artifact-present branch. The same pair on one run, asked for by the same header.
- `aarch64-pc-windows-msvc` (joined) — Windows on ARM64, natively, under Git for Windows' bash. Git's shell runs emulated there and reports `x86_64`, so the installer reads the OS's architecture instead and serves this native build. `install-smoke-sh-windows-arm64` installs the build its producer leg made on an ARM64 Windows runner, reaching its artifact-present branch: the same pair on one run.
