...
2. **Make the scratch repository in-process.** The base is `DEMO_TMP_DIR` when it
   ...
   with `std::fs`, never with `mktemp` — a `contributor` program `--run-demo`
   can spawn but this adopter-floor verb cannot — and a `Drop` guard removes it
   on every exit path, the teardown shape `--run-demo` established. `git init`,
   then a seed commit under a fixed demo identity passed as `-c` flags, so an
   unconfigured host's missing `user.name` cannot stop it.
...
tool-floor roster, context-kit/SPEC.md). **Inferred, cannot run before
build:** the `full` battery is green on a Windows host whose only shell is the
one the platform ships — the PowerShell install-smoke leg that would settle it
drives the demo arm of delta 2, which does not exist until build lands it. The
probes above ran on Linux. If the claim fails there, a gate whose program is
missing exits 2 under the fail-closed contract, so act 2 fails loudly and
...
- docs/index.md's try-it fence gains `npx checkwright demo` as its first command,
  with a trailing comment saying it needs no repository. Its own scratch repo
  makes the fence's `git init`/`git commit` scaffold lines redundant, so those
  two drop; the three that follow (`npx checkwright init`, `git show --stat
  HEAD`, `npx checkwright uninstall`) are unchanged. The prose above the fence
  says that the demo installs nothing, and that the three lines after it are
...
- `installer/SPEC.md` §The verbs (the table and the classifier paragraph),
  §The consumer smoke (the arm roster), and line 716's "The five verbs are not
  ops of this family" sentence, rewritten count-free (deltas 1, 2 and 3).
- `native/src/main.rs`'s count-bearing `// spec:` comment at line 311,
  rewritten count-free (delta 3).
- gate-sdk/SPEC.md §Consumer smoke (the walkthrough paragraph, delta 3).
...
- `the five adopter verbs` — `native/src/installer/mod.rs:1` and
  `native/src/main.rs:311`'s `// spec:` comments, rewritten count-free so a
  sixth verb does not falsify them (delta 3).
- `The five verbs` — installer/SPEC.md:716's opening, rewritten count-free
  (delta 3).

`--run-demo`, `DEMO_TMP_DIR` and the `demo` evidence suite are unaffected names,
not retired ones.
