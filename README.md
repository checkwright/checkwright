...
| [queue-kit/](queue-kit/) | A git-native, agent-readable task tracker: the TASK-QUEUE format, one slug namespace, the tag algebra over tasks (blocked-by/needs-spec/spec/drain-exempt/roadmap/precondition-ok) and over Lessons Learned (an in-iteration attention channel plus consumer-named harvest tags), `queue-index.sh`, a `roadmap.sh` that projects the curated entries onto a generated public page, and the gates that hold the grammar an agent selects work by. |
