...
| [queue-kit/](queue-kit/) | A git-native, agent-readable task tracker: the TASK-QUEUE format, one slug namespace, the tag algebra over tasks (blocked-by/needs-spec/spec) and over Lessons Learned (an in-iteration attention channel plus consumer-named harvest tags), `queue-index.sh`, and the gates that hold the grammar an agent selects work by. |
