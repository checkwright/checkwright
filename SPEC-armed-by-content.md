...
**Measured at authoring (2026-09-23).** `checkwright-gates check-commit-msg README.md /dev/null` prints `COMMIT-MSG: clean (0 banned pattern(s) configured; message unchecked)`, and `check-tree-terms . /dev/null` prints `TREE-TERMS: clean (0 banned pattern(s) configured; tree unchecked)`, each at exit 0. `git grep -n "armed-by:"` over `*/checks/` finds ten declarations, every one in the bare form; the two fixture declarations sit in `check-install-disposition`'s pair. `native/src/installer/doctor.rs`' `disarmed_block` fires only on a member carrying exactly one line whose knob resolves empty. `native/src/gates/install_disposition.rs`' `armed_by_finding` is assertion D.
