...
**Measured at authoring (2026-09-23).** The filed probe `git grep -n -E '== /\*|/\*\)' -- '*.sh'`, and the arm's predicate below run as an ERE over tracked `*.sh`, `*.gate` and `scripts/git-hooks/*`, find the same eight lines: the seven filed sites and `gate-sdk/lib/gate.sh`'s `gate_native_bin_spelled`, which already takes a drive root but not a leading `\`. Every other `/*`-shaped `case` arm or glob in the tracked shell tree is a prefix or containment test and stays green under the predicate: guard.sh's `*/*)`, `"$d"/*)`, `"$d"/* | "./$d"/*)`, `*"$d"/*)`, `*/../*)` and `/dev/*)`, and gate.sh's `https://*|http://*)`.
