...
- **alpha-feature** [spec: SPEC-alpha.md] — an active entry, uncapped and
  uncosted on purpose: an active entry's residency is one iteration by the
  drain rule, so it has no carry to cap and this body may run as long as it
  needs to without the budget gate reaching it at all.
