...
queue() {  # $1=the entry slug, $2=its tag line (empty for none), $3=optional body — writes a minimal well-formed queue
...
queue bad-track "[roadmap: soon/gamma] $decl"
refresh
...
# Assertion C — a tagged entry with no summary reds rather than projecting a
# bullet with no prose.
queue no-decl '[roadmap: soon/alpha]'
refresh
out="$(run_gate)"; code "nodecl-exit" "$?" 1
want "nodecl" "$out" "carries 0 [roadmap-summary:] tag(s)"

# Assertion C — two summaries are ambiguous, so they red too.
queue two-decl "[roadmap: soon/alpha] $decl $decl"
refresh
out="$(run_gate)"; code "twodecl-exit" "$?" 1
want "twodecl" "$out" "carries 2 [roadmap-summary:] tag(s)"

# Assertion C, the other direction — a summary with no roadmap tag is a dead marking,
# which is what a dropped tag looks like from the page's side.
queue dead-marking "$decl"
refresh
out="$(run_gate)"; code "dead-exit" "$?" 1
want "dead-slug" "$out" "dead-marking"
want "dead"      "$out" "no [roadmap:] tag"

# The summary count and the tag fields are independent, so one run reports
# both rather than costing the author a second round trip.
queue both-wrong '[roadmap: eventually/alpha]'
refresh
out="$(run_gate)"; code "both-exit" "$?" 1
want "both-decl"    "$out" "carries 0 [roadmap-summary:] tag(s)"
want "both-horizon" "$out" "unknown horizon 'eventually'"

# Emit grammar. The whitelist is the load-bearing property: only the summary
# is projected, so the entry's summary paragraph, the rest of its body, and the cost
# block all stay off the page. The done section is history, never direction. Each
# bullet links to its entry, the queue relative to the page.
{ printf '## Iteration: demo\n\n## New Features\n\n'
  printf '### live-thing\n\n[roadmap: soon/alpha] [roadmap-summary: The public sentence, and only this.]\n\n'
  printf 'LEADPROSE, an internal one-liner.\n\n'
  ...
  printf '\n## Technical Debt\n\n## Deferred\n\n'
  printf '### deferred-thing\n\n[roadmap: someday/beta] [roadmap-summary: A deferred rung, said publicly.]\n\n'
  printf 'internal framing.\n'
  printf '\n## Done\n\n- shipped-thing\n\n## Lessons Learned\n'
} >"$SANDBOX/TASK-QUEUE.md"
out="$(emit)"
want   "emit-declared"  "$out" '- [live-thing](TASK-QUEUE.md#live-thing) *(alpha)* — The public sentence, and only this.'
absent "emit-leadprose" "$out" "LEADPROSE"
absent "emit-bodyprose" "$out" "BODYPROSE"
absent "emit-costprose" "$out" "COSTPROSE"
want   "emit-deferred"  "$out" '- [deferred-thing](TASK-QUEUE.md#deferred-thing) *(beta)* — A deferred rung, said publicly.'
...
# summary is missing contributes no bullet at all rather than a naked one.
