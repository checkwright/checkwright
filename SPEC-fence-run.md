...
- `native/src/programs.rs`: canon-kit's gate joins `BASH`'s spawners.
  gate-sdk/SPEC.md §The program roster's per-member record for arms gains the
  gate's spawn.
- **The `bash` audience's `derived` predicate gains a third arm** (context-kit/SPEC.md
  §bin/env-probe, `native/src/toolfloor.rs`). No existing audience form keys on
  document content. A kit list, or `registered`, would owe `bash` to every
  canon-kit selection, the `prose` profile included, whether or not a fence is
  marked. The two existing `derived` arms read each kit root on its own, but the
  corpus the gate executes is not inside any kit root. At the default
  `CANON_KIT_SCAN_KIT_ROOTS=0` the manifest set prunes vendored kit roots, so what
  runs is the adopter's own first-party docs. The new arm is therefore read over the
  reader's anchor and not per kit root: **the kit that owns the fence-executing gate
  joins the audience when the anchor's fence-run corpus carries a marked fence.**
  - The owning kit is read off the registry's owner column, never written as a
    literal. It joins only when it is among the reader's kit roots.
  - The corpus and the marker test are the gate's own functions and not copies of
    them. So the arm and the gate cannot disagree about whether a spawn can happen,
    and every `bash` spawn the gate can make is owned by the roster.
  - The sentinel does not change, and neither does the value set or the
    owed-predicate. `derived` is still the conditional form, and only its predicate
    is widened.
  - context-kit's term "bash surface" widens to cover all three arms. That keeps
    installer/SPEC.md §doctor's undecided render (`owed where a kit that ships a
    bash surface is selected`) true with no edit to the installer.
  - A marked fence is not the "prose telling a reader to type a bash command"
    that the bash member's predicate paragraph excludes. The gate itself executes
    the fence, so it is a spawn, which is what the other two arms read.
  - **Honest limit:** a reader's verdict holds for the moment it is read. `init`
    resolves over the payload, which holds no adopter content, so a marker the
    adopter writes later is owned from the next `doctor` or env-probe read. Until
    then, the gate's fail-closed exit 2 on a missing `bash` (delta 3) keeps the gap
    loud.
...
Mechanical: every value is fixed by deltas 1 to 3, and the audience arm is fixed by
the gate's corpus and marker functions.
...
- **The gate's corpus and marker functions (deltas 1 and 3).** Producer: the gate
  module. Consumers: the gate's scan, and the third `derived` arm of the `bash`
  audience (delta 4), which calls them rather than copying them. That arm's own
  readers are the ones `derived` already has: `init`, `doctor`, the env-probe arm
  and `check-install-toolchain`.
...
- gate-sdk/SPEC.md §The program roster, `native/src/programs.rs`,
  `native/src/toolfloor.rs` (the third `derived` arm and its unit test) and
  context-kit/SPEC.md §bin/env-probe: the `derived` bullet of the audience axis, and the
  `bash` member's predicate paragraph (delta 4).
