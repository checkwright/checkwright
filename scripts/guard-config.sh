...
declare -A GUARD_KIT_BREADTH_DECLARED=(
    ["Bash(git worktree *)"]="worktree isolation is mandated rather than optional here — agent-dispatch-guard refuses a read-only-type dispatch that does not carry it — and the harness creates those trees and best-effort auto-cleans them, so the force-removal this breadth reaches destroys only disposable child state — and the iteration boundary now refuses on residue the auto-clean missed (lifecycle-kit/SPEC.md §bin/enter-stage.sh), which makes the reap a required session act rather than an optional one"
