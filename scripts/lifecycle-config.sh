...
# spec: context-kit/SPEC.md §The session-context hook — `session-role`: the marker the hook reads to suppress the delegation nudge for a lead session, whose lifetime is that live session's, not the iteration's
# spec: lifecycle-kit/SPEC.md §templates/lead.md — `lead-journal.md`: the lead's own resume journal, the second artifact of the same live-session lifetime and the third attested victim of this wipe. Both entries are a supervisor's; a stage journal and a consult journal each take none, for the reasons that section contrasts. Cost accepted with the ruled narrow fix: this list covers named members rather than the lifetime, so one stale journal carries across a boundary crossed with no lead live, until the next lead overwrites it.
LIFECYCLE_KIT_BOUNDARY_PRESERVE=(session-role lead-journal.md)
