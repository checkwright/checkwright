#!/usr/bin/env bash
# spec: spec-kit/SPEC.md §check-prose-enum — this repo's enum-set emitter: the queue tag vocabulary, derived (not restated) from queue-kit's own lead-line tag parser plus the configured lesson tags. One <set-name><TAB><member> line per member. Two sets, because the tags partition by role: the task/selection tags list together in prose, the Lessons channel tags list together — a paragraph naming one role's tags is not enumerating the other.
set -uo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

# spec: spec-kit/SPEC.md §check-prose-enum — the tag vocabulary is queue-kit's own parse surface: the arr["<tag>"] rows check-tag-lead-line keys on, read from the gate rather than re-listed here
...
# spec: spec-kit/SPEC.md §check-prose-enum — the Lessons channel: [attend] (queue-kit/SPEC.md §The Lessons Learned channel) plus this repo's configured harvest tags; the rest are task/selection tags
