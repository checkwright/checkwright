...
# spec: drift-kit/SPEC.md §Layout and configuration — DRIFT_KIT_STAGES derives from lifecycle-kit's LIFECYCLE_KIT_STAGES (the sole roster owner) so the trajectory table renders exactly this repo's live roster; a parallel literal here would reintroduce the roster-drift this wiring exists to eliminate. The read costs a spawn, so it runs only for a batch resolving DRIFT_KIT_STAGES.
case $'\n'"${GATE_SDK_RESOLVING_KNOB:-}"$'\n' in
    *$'\n'DRIFT_KIT_STAGES$'\n'*) gate_static_knob LIFECYCLE_KIT_STAGES DRIFT_KIT_STAGES || exit 2 ;;
esac
