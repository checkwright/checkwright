...
is holding it, such as a background child outliving the work (a self-matching
waiter, say). Check the session's leftover processes (waiters, shells,
...
**A recurrence you judge is still yours to record, and your discharge differs.**
The no-queue-write rule above leaves you no way to stamp one, so you **file the
judgment and its grounds into a gap bullet** (`--emit file-gap`) and the session
that may write the queue stamps from your prose — at the boundary as safely as
mid-iteration (lifecycle-kit/SPEC.md §The committed gap inbox). What you must not
do is leave the judgment unrecorded on the ground that you cannot stamp it.
