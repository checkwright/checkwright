...
# no stamp, the rename subject, and the no-arg, missing-file and wrong-arity edges.
...
run() {  # $1=subject $2=staged $3=head
    printf '%s\n' "$1" > "$tmp/msg.txt"
    (gate_run check-stamp-subject "$DIR/checks" "$tmp/msg.txt" "$2" "$3" >/dev/null 2>&1)
    echo $?
}
...
echo "check-stamp-subject.test: ok (boundary reset; rename; no stamp; unscoped; edges)"
exit 0
