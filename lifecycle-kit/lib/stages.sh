...
[[ -v LIFECYCLE_SHIM_NGRAM ]] || LIFECYCLE_SHIM_NGRAM=9
declare -p LIFECYCLE_SHIM_DEDUP_CORPUS &>/dev/null || LIFECYCLE_SHIM_DEDUP_CORPUS=()
...
[[ "$LIFECYCLE_SHIM_NGRAM" =~ ^[1-9][0-9]*$ ]] \
    || _lc_errs+=("LIFECYCLE_SHIM_NGRAM '$LIFECYCLE_SHIM_NGRAM' is not a positive integer")
