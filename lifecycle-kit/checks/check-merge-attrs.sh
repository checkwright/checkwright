...
# spec: lifecycle-kit/SPEC.md §check-merge-attrs — bidirectional parity between the derived iteration-scoped supersede set and the merge=iteration-scoped lines in .gitattributes; a smuggled ours-driver attribute on a path outside the set is the reverse-direction safety edge
#
...
if [[ -n "$missing" || -n "$extra" ]]; then
    echo "check-merge-attrs: $ATTRS merge=iteration-scoped lines are out of parity with the derived iteration-scoped supersede set:"
    ...
    echo "  help: regenerate the marker block — bash lifecycle-kit/bin/install-lifecycle.sh — which writes exactly one 'merge=iteration-scoped' line per boundary-truncated surface (LIFECYCLE_KIT_STATE_FILE, LIFECYCLE_KIT_LESSON_EVIDENCE_FILE, and each LIFECYCLE_KIT_BOUNDARY_TRUNCATE member). Remove any hand-added attribute on a path outside that set."
    exit 1
fi

dcount="$(grep -vc '^$' <<<"$derived")"
echo "MERGE-ATTRS: clean ($ATTRS carries a merge=iteration-scoped line for each of the $dcount derived iteration-scoped surface(s), and no others)"
exit 0
