...
**Batch condition, with `stamp-subject-merge-carve-out-unruled` (merged into §check-stamp-subject).** That entry widens §check-stamp-subject's added-stamp read to exclude lines a merge parent carries. If it lands in this batch or an earlier one, this gate calls the same read, merge parents included, from one shared function in the gate modules. If this entry lands first, the gate reads the prior set from `HEAD`'s version alone, as §check-stamp-subject does today, and the later merge moves both gates onto the shared read in one commit.
