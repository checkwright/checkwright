...
# spec: doctrine-kit/SPEC.md §install-doctrine — the installer is a non-gate arm of the vendored gate
# binary after the behind-invoke relocation, so the smoke reaches it through the front-end exactly as
# a consumer does, rather than through a shell entry point the kit no longer ships
install_doctrine() { bash "$SDK/bin/run-gates.sh" --install-doctrine "$@"; }
...
SMOKE_ERR="$(install_doctrine 2>&1 >/dev/null)"
